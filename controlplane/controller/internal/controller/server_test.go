...
func TestGetConfigHash(t *testing.T) {
	...
	controller.swapCache(cache)

	t.Run("returns valid sha256 hash", func(t *testing.T) {
		resp, err := agent.GetConfigHash(ctx, &pb.ConfigRequest{Pubkey: "abc123"})
		if err != nil {
			t.Fatalf("GetConfigHash returned error: %v", err)
		}
		if len(resp.Hash) != 64 {
			t.Errorf("expected 64-character hex hash, got %d characters: %s", len(resp.Hash), resp.Hash)
		}
		// Verify it's valid hex
		_, err = hex.DecodeString(resp.Hash)
		if err != nil {
			t.Errorf("hash is not valid hex: %v", err)
		}
	})

	t.Run("hash matches sha256 of GetConfig response", func(t *testing.T) {
		configResp, err := agent.GetConfig(ctx, &pb.ConfigRequest{Pubkey: "abc123"})
		if err != nil {
			t.Fatalf("GetConfig returned error: %v", err)
		}
		hashResp, err := agent.GetConfigHash(ctx, &pb.ConfigRequest{Pubkey: "abc123"})
		if err != nil {
			t.Fatalf("GetConfigHash returned error: %v", err)
		}
		expectedHash := sha256.Sum256([]byte(configResp.Config))
		expectedHashStr := hex.EncodeToString(expectedHash[:])
		if hashResp.Hash != expectedHashStr {
			t.Errorf("GetConfigHash hash %q does not match SHA256 of GetConfig config %q", hashResp.Hash, expectedHashStr)
		}
	})

	t.Run("hash is stable across calls", func(t *testing.T) {
		resp1, err := agent.GetConfigHash(ctx, &pb.ConfigRequest{Pubkey: "abc123"})
		if err != nil {
			t.Fatalf("first GetConfigHash returned error: %v", err)
		}
		resp2, err := agent.GetConfigHash(ctx, &pb.ConfigRequest{Pubkey: "abc123"})
		if err != nil {
			t.Fatalf("second GetConfigHash returned error: %v", err)
		}
		if resp1.Hash != resp2.Hash {
			t.Errorf("expected stable hash across calls, got %q and %q", resp1.Hash, resp2.Hash)
		}
	})

	t.Run("hash changes when config changes", func(t *testing.T) {
		resp1, err := agent.GetConfigHash(ctx, &pb.ConfigRequest{Pubkey: "abc123"})
		if err != nil {
			t.Fatalf("GetConfigHash returned error: %v", err)
		}

		// Update cache with an additional tunnel
		updatedCache := cache
		updatedDevices := map[string]*Device{
			"abc123": {
				PubKey:                "abc123",
				Code:                  "dz1",
				Interfaces:            []Interface{},
		...
		updatedCache.Devices = updatedDevices
		controller.swapCache(updatedCache)

		resp2, err := agent.GetConfigHash(ctx, &pb.ConfigRequest{Pubkey: "abc123"})
		if err != nil {
			t.Fatalf("GetConfigHash returned error after update: %v", err)
		}
		if resp1.Hash == resp2.Hash {
			t.Error("expected hash to change after config update, but it stayed the same")
		}
	...
	t.Run("returns error for unknown pubkey", func(t *testing.T) {
		_, err := agent.GetConfigHash(ctx, &pb.ConfigRequest{Pubkey: "unknown"})
		if err == nil {
			t.Fatal("expected error for unknown pubkey, got nil")
		}
	})
}
