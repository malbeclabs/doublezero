...
- CI
  ...
  - `dispatch-and-wait` now reports the names of the failing jobs to Slack when a dispatched cross-repo run fails, so the release thread says what broke instead of only that something did. The report is best-effort: it warns and exits 0 on a token or API failure, and posts a "no failing jobs were reported" note for a cancelled run or a run that failed with no failed job, so it can never mask or replace the failure the calling job already reports. The job list is capped at 10 names with an overflow count. Wired up on the orchestrator's `qa` job (the infra `qa.testnet.yml` dispatch), which now also lists `preflight` directly in `needs` to read its `thread_ts`. (#4121)
