...
- rfcs: add RFC-21 proposing the extension of `Tenant` from a unicast-only construct to a per-use-case resource grouping primitive (capabilities flag, optional `tenant_pk` on multicast groups, widened access-pass authorization for tenant administrators)
- Smartcontract
