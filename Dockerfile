...
# Its main package spans more than one file, so this builds the package, not a single file.
RUN --mount=type=cache,target=/go/pkg/mod \
    --mount=type=cache,target=/root/.cache/go-build \
    go build -ldflags "${GO_LDFLAGS}" -o ${BIN_DIR}/doublezero-internet-latency-collector ./controlplane/internet-latency-collector/cmd/collector
