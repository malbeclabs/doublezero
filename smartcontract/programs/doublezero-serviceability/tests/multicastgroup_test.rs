...
async fn test_multicastgroup_create_with_wrong_index_fails() {
    ...
    let (wrong_multicastgroup_pubkey, wrong_bump_seed) =
        get_multicastgroup_pda(&program_id, wrong_index);
    ...
    let (correct_multicastgroup_pubkey, correct_bump_seed) =
        get_multicastgroup_pda(&program_id, correct_index);
