...
func runTest_SolanaStakeShareDecrease(t *testing.T, llmFactory LLMClientFactory) {
	...
	// Set up agent with LLM client (use more rounds for complex stake share queries)
	agentInstance := setupAgent(t, ctx, db, llmFactory, debug, debugLevel, &react.Config{
		MaxRounds: 15,
	})
