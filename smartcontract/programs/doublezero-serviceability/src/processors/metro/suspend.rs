use crate::{
    ...
    state::{globalstate::GlobalState, metro::*},
};
...
pub struct MetroSuspendArgs {}

impl fmt::Debug for MetroSuspendArgs {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "")
    }
}

pub fn process_suspend_metro(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    _value: &MetroSuspendArgs,
) -> ProgramResult {
    ...
    #[cfg(test)]
    msg!("process_suspend_metro({:?})", _value);
    ...
    // Authorization:
    //  - Only accounts in the foundation_allowlist may suspend the metro.
    ...
    metro.status = MetroStatus::Suspended;
