...
use clap::Args;
use doublezero_sdk::commands::facility::get::GetFacilityCommand;
use serde::Serialize;
use std::io::Write;
use tabled::Tabled;

#[derive(Args, Debug)]
pub struct GetFacilityCliCommand {
    /// Facility Pubkey or code to get details for
...
impl GetFacilityCliCommand {
    pub fn execute<C: CliCommand, W: Write>(self, client: &C, out: &mut W) -> eyre::Result<()> {
        let (pubkey, facility) = client.get_facility(GetFacilityCommand {
        ...
        let display = LocationDisplay {
            ...
            lat: facility.lat,
            lng: facility.lng,
            loc_id: facility.loc_id,
            reference_count: facility.reference_count,
            status: facility.status.to_string(),
...
#[cfg(test)]
mod tests {
    use crate::{facility::get::GetFacilityCliCommand, tests::utils::create_test_client};
    use doublezero_sdk::{AccountType, Facility, FacilityStatus, GetFacilityCommand};
    ...
    fn test_cli_location_get() {
        ...
        assert_eq!(json["name"].as_str().unwrap(), "Test Facility");
