...
import (
	...
	"github.com/malbeclabs/doublezero/telemetry/sflow-proxy/internal/metrics"
...
func (s *Server) Run(parentCtx context.Context) error {
	s.log.Info("starting sflow proxy server",
...
func (s *Server) healthLoop(ctx context.Context) error {
	...
	for {
		...
		if isClosedNetErr(err) {
			metrics.HealthAcceptErrs.WithLabelValues("closed").Inc()
		} else {
			metrics.HealthAcceptErrs.WithLabelValues("other").Inc()
		}
		...
		// Best-effort: don’t take down ingest if health accept is temporarily failing.
		s.log.Warn("health listener accept error; backing off", "error", err, "backoff", backoff)
