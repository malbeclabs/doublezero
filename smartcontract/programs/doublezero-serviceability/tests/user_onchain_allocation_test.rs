...
//!
//! Tests cover:
//! - Atomic CreateUser with on-chain allocation
//! - DeleteUser with resource deallocation
//! - RequestBanUser with resource deallocation
//! - UpdateUser onchain (tunnel_id, tunnel_net, dz_ip) reallocations
//! - Multicast publisher/subscriber count side-effects
...
use doublezero_serviceability::{
    ...
    processors::{
        ...
        user::{
            create::UserCreateArgs, delete::UserDeleteArgs, requestban::UserRequestBanArgs,
...
/// Helper: atomic create+allocate+activate for any UserType.
...
// ============================================================================
// MulticastPublisherBlock deallocation / counter tests
// ============================================================================

/// Resource-lifecycle regression: a publisher user allocates `dz_ip` out of
/// the MulticastPublisherBlock on subscribe; atomic delete deallocates it
/// back to the block. After delete, the block has zero allocations and a
/// fresh user subscribing as publisher allocates from the same range — i.e.,
/// the block is reusable end-to-end.
#[tokio::test]
async fn test_multicast_publisher_block_deallocation_and_reuse() {
    ...
    // CreateMulticastGroup(use_onchain_allocation: true) atomically activates;
    // no separate ActivateMulticastGroup is needed.
    ...
    // Subscribe user_1 as publisher (onchain). Allocates dz_ip from MulticastPublisherBlock.
    let recent_blockhash = wait_for_new_blockhash(&mut banks_client).await;
    execute_transaction(
        &mut banks_client,
        recent_blockhash,
    ...
    let user_1 = get_account_data(&mut banks_client, user_pubkey)
        ...
        .get_user()
        .unwrap();
    let first_dz_ip = user_1.dz_ip;
    assert_eq!(first_dz_ip.octets()[0..2], [148, 51]);
    let mpb_after_sub =
        get_resource_extension_data(&mut banks_client, multicast_publisher_block_pubkey)
            .await
            .unwrap();
    assert_eq!(
        mpb_after_sub.iter_allocated().len(),
        1,
        "MulticastPublisherBlock should have one allocation after publisher subscribe"
    );

    // Unsubscribe to clear user_1.publishers (delete guard requires empty publishers).
    // Onchain unsubscribe deallocates dz_ip and resets it to client_ip.
    let recent_blockhash = wait_for_new_blockhash(&mut banks_client).await;
    execute_transaction(
        &mut banks_client,
        recent_blockhash,
    ...
    let mpb_after_unsub =
        get_resource_extension_data(&mut banks_client, multicast_publisher_block_pubkey)
            ...
            .unwrap();
    assert!(
        mpb_after_unsub.iter_allocated().is_empty(),
        "MulticastPublisherBlock should be empty after publisher unsubscribe"
    );

    // Atomic delete user_1. dz_ip is already back to client_ip, so
    // multicast_publisher_count = 0 (no MulticastPublisherBlock account passed).
    let recent_blockhash = wait_for_new_blockhash(&mut banks_client).await;
    execute_transaction(
        &mut banks_client,
        recent_blockhash,
    ...
    let mpb_after_delete =
        get_resource_extension_data(&mut banks_client, multicast_publisher_block_pubkey)
            .await
            .unwrap();
    assert!(
        mpb_after_delete.iter_allocated().is_empty(),
        "MulticastPublisherBlock should still be empty after atomic delete"
    );

    // Re-AccessPass + atomic create a second user on a different client_ip, subscribe
    // as publisher, and verify MulticastPublisherBlock allocates again (block is reusable).
    let client_ip_2 = [100, 0, 0, 21];
    let (accesspass2_pubkey, _) =
        get_accesspass_pda(&program_id, &client_ip_2.into(), &payer.pubkey());
    let recent_blockhash = wait_for_new_blockhash(&mut banks_client).await;
    execute_transaction(
        &mut banks_client,
        recent_blockhash,
    ...
    let (user2_pubkey, _) = get_user_pda(&program_id, &client_ip_2.into(), UserType::Multicast);
    let recent_blockhash = wait_for_new_blockhash(&mut banks_client).await;
    execute_transaction(
        &mut banks_client,
        recent_blockhash,
    ...
    let user_2 = get_account_data(&mut banks_client, user2_pubkey)
        .await
        .expect("User 2 not found")
        .get_user()
        .unwrap();
    // dz_ip allocated from MulticastPublisherBlock range (148.51.120.0/21).
    assert_eq!(user_2.dz_ip.octets()[0..2], [148, 51]);
    let mpb_after_reuse =
        get_resource_extension_data(&mut banks_client, multicast_publisher_block_pubkey)
            .await
            .unwrap();
    assert_eq!(
        mpb_after_reuse.iter_allocated().len(),
        1,
        "MulticastPublisherBlock should have one allocation again after reuse"
    );
    println!(
        "[PASS] block reused; first allocated dz_ip={}, second dz_ip={}",
        first_dz_ip, user_2.dz_ip
    );
}

/// Edge case for the `CreatedAsPublisher` flag: a multicast user created as a
/// subscriber (`tunnel_flags = 0`) transiently subscribes as a publisher and
/// then unsubscribes. The atomic delete must decrement `multicast_subscribers_count`
/// (the create-time bucket), not `multicast_publishers_count`, because the
/// `CreatedAsPublisher` flag was never set.
#[tokio::test]
async fn test_delete_user_atomic_decrements_subscribers_count_for_non_publisher() {
    ...
    // Transient publisher subscribe — CreatedAsPublisher stays unset.
    let recent_blockhash = wait_for_new_blockhash(&mut banks_client).await;
    execute_transaction(
        &mut banks_client,
        recent_blockhash,
    ...
    let user_after_sub = get_account_data(&mut banks_client, user_pubkey)
        .await
        .expect("User not found")
        .get_user()
        .unwrap();
    assert!(
        !TunnelFlags::is_set(user_after_sub.tunnel_flags, TunnelFlags::CreatedAsPublisher),
        "CreatedAsPublisher must stay unset for a user created as subscriber"
    );

    // Unsubscribe (publishers becomes empty so delete guard passes).
    let recent_blockhash = wait_for_new_blockhash(&mut banks_client).await;
    execute_transaction(
        &mut banks_client,
        recent_blockhash,
    ...
    // Counter sanity before delete.
    let device_before_delete = get_account_data(&mut banks_client, device_pubkey)
        .await
        .expect("Device not found")
        .get_device()
        .unwrap();
    assert_eq!(device_before_delete.multicast_subscribers_count, 1);
    assert_eq!(device_before_delete.multicast_publishers_count, 0);
    ...
    assert_eq!(
        device_after.multicast_subscribers_count, 0,
        "subscribers_count must decrement on delete (user created as subscriber)"
    );
    assert_eq!(
        device_after.multicast_publishers_count, 0,
        "publishers_count must remain 0 (CreatedAsPublisher never set)"
    );
}
