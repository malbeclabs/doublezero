...
- Client
  - `doublezerod` takes an optional `client_ip` on `POST /enable`, which pins the address the reconciler matches onchain users against and, for plain IBRL, uses verbatim as the GRE tunnel source. Without it the flag could not work at all: the CLI stopped provisioning tunnels when the daemon's reconciler took that over, so a CLI-supplied address reached nothing, and a user created at one address while the daemon watched another would never be provisioned or torn down. The daemon repeats the local-assignment check rather than trusting the caller, and fails the request when it cannot verify the address, since nothing downstream is left to catch it. A pin on an already-enabled reconciler still reconciles, tearing down the previous address's services and provisioning the new one, and drops the cached tunnel sources resolved for the old address. The pin is persisted alongside the reconciler-enabled state and restored on restart, so a reboot does not revert the host to its discovered address and take the tunnel with it; the daemon's own `-client-ip` flag still outranks it. It is host configuration rather than session state: every state write persists the pin in effect rather than the request being served, so an enable without one leaves it alone, a disable leaves it alone, and only a new pin replaces it — the running daemon and the state file cannot disagree about which address the host presents. The daemon refuses a pin that is not globally routable as well as one this host does not hold: no user can exist onchain at a non-global address, so such a pin would match nothing, tear down the host's services and survive every restart. A pin restored on startup is re-checked against this host's interfaces first: the check `/enable` made says nothing about now, and a host whose address left with a DHCP lease, a NIC swap or a re-addressing would come back up pinned to something the kernel does not hold, matching no onchain user and building no tunnel, with no un-pin path short of editing the state file. Such a pin falls back to discovery with a warning and is dropped rather than kept dormant: held, it would be reported as the pin in effect, written back by the next state write, and applied again on some later restart once the address happened to return. An enumeration failure keeps it — a failure to check is not a failed check.
- SDK
