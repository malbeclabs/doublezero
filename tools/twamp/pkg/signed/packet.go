...
//
// Both ProbePacket and ReplyPacket verify against a pubkey taken straight off
// the wire, and ed25519.Verify does not screen small-order keys: with S and R
// zero the equation [S]B = R + [k]A reduces to identity = R + [k]A, which holds
// for the all-zero key on ~24% of messages and for the identity encoding
// (0x01||00*31) on *every* message — so an unsigned datagram verifies. Rejecting
// a zero S closes the whole class rather than blacklisting known-bad keys:
// [S]B lies in the prime-order subgroup generated by B, a small-order R + [k]A
// lies in the torsion subgroup, and those meet only at the identity, so any
// forgery of this shape needs S ≡ 0 (mod L) — and Verify already requires S to
// be canonical, making S exactly zero. A nonzero S is as hard to produce as an
// ordinary forgery, small-order key or not.
