...
func TestInternetLatency_CSVExporter_New(t *testing.T) {
	...
	expectedHeader := "source_metro_code,target_metro_code,timestamp,latency\n"
...
func TestInternetLatency_CSVExporter_WriteRecords(t *testing.T) {
	...
	records := []exporter.Record{
		{
			SourceMetroCode: "source1",
			TargetMetroCode: "target1",
			Timestamp:       time.Unix(100, 0),
			RTT:             time.Duration(100),
		},
		{
			SourceMetroCode: "source2",
			TargetMetroCode: "target2",
