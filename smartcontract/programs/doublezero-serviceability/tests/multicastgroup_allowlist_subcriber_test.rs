...
/// Feed authority can add to subscriber allowlist on an access pass it does NOT own.
#[tokio::test]
async fn test_multicast_subscriber_allowlist_feed_authority_non_owner_add() {
    let (mut banks_client, program_id, payer, recent_blockhash) = init_test().await;

    let client_ip = [100, 0, 0, 5].into();
    let user_payer = payer.pubkey();
    ...
    // 4. Payer (NOT feed) creates access pass — payer is the owner
    let (accesspass_pubkey, _) = get_accesspass_pda(&program_id, &client_ip, &user_payer);
    ...
    assert_eq!(
        accesspass.owner,
        payer.pubkey(),
        "Payer should own the access pass"
    );
    ...
    assert!(
        res.is_ok(),
        "Feed authority should add to allowlist on access pass it does not own"
    );
