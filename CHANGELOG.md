...
- CLI
  - Move the `connect` verb into the `doublezero-daemon-cli` crate per RFC-20. It now takes `&CliContext` + generic `&D: DaemonClient` + `&L: LedgerClient` + `&mut W` writer; informational/result lines route through the shared writer, spinners stay on stderr, and device selection uses the crate's latency utilities (from #3995). The binary's `dzd_latency.rs` and the orphaned `check_doublezero` pre-flight are deleted. Behavior (flags, output, `--verbose`, version-check) is unchanged.
