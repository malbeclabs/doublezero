...
- CLI
  ...
  - Declaring `ibrl: <tenant>` also pins `last_access_epoch` to unlimited, since the tenant and the epoch are one grant and a finite value fails `connect ibrl` at an unpredictable date. The write reads the pass first and re-sends the type, seat caps and `allow_multiple_ip` unchanged, because `access-pass set` overwrites those from its arguments; it targets the stored pass, so a grant on a shared `0.0.0.0` pass is not written to a different account. Clearing a tenant sends `last_access_epoch: 0` when the stored epoch has already elapsed, because `access-pass set` rejects an elapsed epoch and IBRL would otherwise be impossible to revoke from an expired pass; an epoch still in the future is preserved.
  - A declared subscribe is written to the subscriber allowlist even when a feed seated on the pass carries the group. Feed coverage authorizes a join only at connect and only in the feed's own metro, while `doublezero multicast subscribe` goes through `UpdateMulticastGroupRoles`, which checks the allowlist for every role gained whatever the pass type. The coverage is reported as a warning instead.
  - Both verbs exit non-zero when the plan contains blocked items, so a CI job can gate `apply` on `plan`. `apply` refuses a blocked plan before the confirmation prompt and sends nothing, rather than converging the valid subset; `--allow-blocked` applies the rest and still exits non-zero. `apply --dry-run` writes nothing and so requires neither a keypair nor credits, and its `--json` `changed` field reports whether the plan *would* move anything. A document saying `user_payer: me` is refused when no signer is configured, rather than resolving to the default pubkey, and an empty multicast group code is rejected with its entry index.
  - Two cases are blocked rather than attempted: an access pass that does not exist (granting against an empty PDA would silently mint a `Prepaid` pass with 1/1 seats and no epoch), and two entries that resolve to the same shared pass while declaring different access (one account cannot hold both, so the two would flip the state on every run). Entries that resolve to one pass and agree are planned once.
  - New `access-pass remove-feed --user-payer <pubkey> --feed <name|pubkey>` removes one feed from an EdgeSeat access pass while its other feeds stay active, for example when a trial ends. It prints the plan and asks for confirmation, and `--dry-run` prints the plan without sending anything. It drops the feed's seat first, so no new user can subscribe to the feed. It then unsubscribes each holder that keeps another feed and deletes each holder left with no feed. When a delete fails, it prints the `user delete` command that finishes the job. Last, it lowers the multicast user cap to the sum of the remaining seats. The pass's subscriber allowlist is left alone, because feeds grant their groups without it.
- Client
...
- Geolocation
  - geoprobe-target bounds offset replay with a per-key `MeasurementSlot` floor (#4286). A signature stays valid forever, so an offset captured off the wire could be replayed into `location_offsets` — which the lake explorer publishes — indefinitely. `MeasurementSlot` is inside the signed payload, so the highest slot a signing key has proven is a lower bound on real time that a replay can repeat but cannot advance without the private key; offsets more than 2 minutes below that floor are dropped, as are repeats once the floor has stood still for 30 minutes. The floor also refuses to advance further than elapsed wall time allows, so one anomalous slot — a probe pointed at the wrong ledger RPC needs no attacker — is rejected rather than ratcheted in, which would have locked that key out until the process restarted. A never-seen key's first offer is checked the same way against the highest slot other live keys have proven, because a floor seeded at the wrong cluster's height is out of reach of every genuine offer that follows; a target hearing from a single sender has no such reference and still needs a restart in that case. Floors are keyed by `AuthorityPubkey`, the key the signature is verified against, not by the signed-but-unauthenticated `SenderPubkey` — otherwise one minted keypair stamping a real geoprobe's `SenderPubkey` with a huge slot would lock that geoprobe out of the table. The floor is derived from the offset stream rather than a ledger clock because geoprobe-target holds no RPC connection by design. In steady state a sender's floor advances every ~5 minutes, so the replay window is about 7 minutes; the 30-minute stall bound only governs the degraded case where the sending probe is riding out its own RPC outage on a frozen cached slot. Rejections log at Warn with a machine-readable `reason`, the authority and sender pubkeys, the offered slot, the floor and the floor's age — geoprobe-target exposes no metrics, so these lines are the only signal that a sender stopped being ingested.
  ...
  - `MinCache.Update` no longer replaces `best` on an equal RTT, matching the geoprobe agent's own cache, so an equal repeat stops restarting `best`'s expiry clock on the fast path. The slot floor, not this, is what bounds replay.
  - geoprobe-target drops LocationOffsets whose signature chain does not verify instead of caching them and writing them to `location_offsets`, which the lake explorer publishes unfiltered. Verification also rejects a signature whose S scalar is zero, which no real signature has and every forgery against a small-order Ed25519 key does: `ed25519.Verify` does not screen those keys, so an unsigned datagram verifies on ~24% of messages with an all-zero pubkey and on *every* message with the identity encoding. The same guard covers the signed TWAMP `ProbePacket.Verify` and `ReplyPacket.Verify`, the other two places a wire-supplied pubkey is used as the verification key.
  - The geoprobe agent enforces RFC-16's replay mitigation: an inbound DZD offset is rejected unless its `MeasurementSlot` is within 15 minutes behind or 10 minutes ahead of the reference slot. The lead is derived from how stale that reference may be, so a DZD stamping a current slot is not mistaken for a replay while the reference lags. An equal-RTT offset also no longer replaces the cache's best entry, because replacing it reset its expiry clock and let a replay pin the probe's reference point indefinitely.
...
- Collector
  - The Wheresitup collector drops tracked jobs whose results WheresItUp has already discarded instead of polling them, and asks for a one-hour expiry rather than ten minutes. Past `expire_after` the API keeps returning a job with an empty `complete` and a populated `in_progress`, the shape of a job still running, so expired jobs were polled every cycle: a vendor stall stretched the pass until every new job was first polled after its own expiry and was retained in turn, which left mainnet-beta's wheresitup provider reporting no samples for about 22 hours. Missing samples are now counted on every cycle rather than only on cycles that exported a record, which is the signal that was absent throughout. Dropped jobs increment a counter of their own, since an expired job leaves the pending gauge. A cycle whose location mapping comes back empty, the shape of a failed ledger fetch, now fails instead of exporting every record as `Unknown` and dropping the jobs, so the results survive to the next cycle.
  ...
  - A probe that fails as a RIPE Atlas measurement target no longer bars itself from sourcing measurements. Target failures go to their own list, honored by target selection only, except a target whose measurement never produced a single result, which is treated as offline and leaves the source pool too; a probe that stopped running measurements altogether still bars both roles. Failing to answer pings says nothing about sending them, and the single shared list meant un-blacklisting one metro's dead target probe changed that metro's source probe in three unrelated measurements and tore all three down. (malbeclabs/doublezero#4331)
...
- CLI
  - `doublezero feed create --chain` and `doublezero feed update --chain` take `solana`, `hyperliquid`, or `unspecified`. Create without the flag writes `Unspecified`. Update without the flag leaves the stored value. `doublezero feed update --pubkey` is repeatable and packs up to 8 feeds into one ledger transaction. `--name` and `--group` still take one feed. `doublezero feed list` prints the chain after the name. (#4353)
- SDK
  - `CreateFeedCommand` and `UpdateFeedCommand` carry the chain. `UpdateFeedCommand` takes a list of pubkeys, sends one `UpdateFeed` per feed, and refuses more than 8 in one transaction. Go, Python, and TypeScript decoders read the trailing byte and decode a short account as `Unspecified`. The feed fixture ends with `Hyperliquid` (2). (#4353)
...
- Serviceability
  ...
  - `Feed` carries `feed_chain`: `Unspecified` 0, `Solana` 1, `Hyperliquid` 2. A feed written before the field decodes as `Unspecified`. `CreateFeed` writes the caller's value and accepts `Unspecified`. `UpdateFeed` changes it only when the argument is set, and grows a short account when it writes. (#4353)
...
- SDK
  - `ActivateFeedCommand`, `HaltFeedCommand`, `ResumeFeedCommand`, `RetireFeedCommand` and `FinalizeFeedRetirementCommand`, and the matching methods on the CLI's `CliCommand` trait. `FinalizeFeedRetirementCommand` appends no `Permission` account, unlike every other feed command: finalizing is permissionless, so the clock decides rather than a key, and sending one would ask the program for an account it does not read.
- CI
  - Every SBF build installs the platform-tools it asks for before building, through `scripts/install-sbf-tools.sh`. `cargo build-sbf --tools-version v1.54` only honors the flag when it can prove that version exists: it looks under `~/.cache/solana` and, finding nothing, resolves the anza `releases/latest` redirect. When the requested version is newer than whatever that redirect names, it logs at a level nobody sees and falls back to its own built-in v1.51, whose Cargo is 1.84. anza published v1.51.1 after v1.54, so the redirect now names v1.51.1 and every `--tools-version v1.54` build took the fallback. The failure surfaced nowhere near the flag: Cargo 1.84 cannot parse the edition2024 crates in the solana 3.0 tree, so `rust-build`, `rust-test` and `rust-validator-test` all died on `feature \`edition2024\` is required` in `offchain/crates/contributor-rewards`, a workspace member no program depends on. Installing the requested version first takes the network lookup out of the decision, which also means the tools no longer vary with what anza tagged most recently. The installer replaces a cache left half-extracted by an interrupted build, which is what the e2e base image's own `rust/lib` check did by deleting it, so that check gives way to the install. A tree is complete only when it carries `rust/bin/cargo`, `rust/bin/rustc`, `rust/lib` and `llvm/bin/clang`: cargo-build-sbf exports platform-tools' clang, llvm-ar, llvm-objdump and llvm-objcopy as `CC`, `AR`, `OBJDUMP` and `OBJCOPY`, and this tree compiles C, so an extraction that stopped between `rust/` and `llvm/` would otherwise read as installed and fail every later build on a missing clang. The install holds a lock for the whole of its work, because the e2e images share one `~/.cache/solana` across concurrent builds on a self-hosted runner: unserialized, one build can delete the tree another is reading, and a move landing on a directory recreated in the gap nests instead of replacing, leaving a version directory that is not empty and therefore reads as installed.
  - The Agave toolchain action reports which `cargo build-sbf` the job resolved and what version it is, and removes any `cargo-build-sbf` or `cargo-test-sbf` in `$CARGO_HOME/bin`, which Cargo would prefer over the one just installed. The same build has come out both ways here: one run drives the host Cargo and tolerates the fallback, the next drives the platform-tools Cargo and dies on edition2024, and the logs gave no way to tell which binary ran. The removal is a guard against one way that can happen; no run has been seen carrying such a copy, so it is not the whole explanation. The version it reports is the `rustc` in each `~/.cache/solana` entry rather than `cargo build-sbf --version`, which formats its banner from a compile-time constant before parsing arguments and so reads `v1.51` on a fixed run and a broken one alike. Reading those binaries by path also keeps the step from invoking `cargo` at all, which had been making rustup fetch the repository-root toolchain for jobs that pin their own: a composite action does not inherit a workflow's `defaults.run.working-directory`.
- Serviceability
  - A test that drives the RFC-28 sequence end to end: a relayer writes a `StakeMirror`, a builder holding no permission creates a feed against it, grants its own publisher the right to send, and an operator activates it. First test to reach `ActivateFeed` from a real `CreateFeed` on a mirror a real `WriteStakeMirror` produced. Nothing is seeded.
  - `ActivateFeed` (variant 124), which admits a feed that was waiting on a conformance verdict and moves it `Pending` to `Active`. Nothing could make that transition before: there was no processor for it and `UpdateFeed` writes `name` and `groups` only, so a staked feed sat in `Pending` for life, selling no seats and admitting no subscribers. Only from `Pending`, since `Halted` has `ResumeFeed` and nothing leaves `Retired`; every other status is refused as `FeedNotActivatable` (130). A `FEED_AUTHORITY` or `FOUNDATION` key signs it and the feed's own builder is refused by name before those roles are read, because a builder may also hold one of them and `authorize` would admit it on that strength, which is the one feed instruction where that asymmetry is deliberate: halt and resume are the builder's levers under RFC-28, but a builder that could admit its own feed would be attesting to its own conformance, which is what `Pending` exists to prevent. Activating re-reads the stake mirror and refuses a feed whose tier no longer covers its rate, the same check `ResumeFeed` makes and for the same reason: `CreateFeed` checked the cover it was shown, and a relayer can write a smaller tier afterwards, so this is the only place a correction reaches a feed that a bad mirror already admitted. The args carry no verdict, because nothing reads one: the grounds a verdict would record are what programmatic slashing acts on, and no slash instruction exists yet, so fixing the payload's shape now would fix it at the point we know least. `activate_feed` joins the instruction crate's feed builders.
  - A covering stake authorizes `CreateFeed` on its own, so a builder holding no permission deploys a feed against the bond it posted. The gate reads the mirror rather than trusting one is there: the address is derived from `stake_ref`, a caller-supplied argument, so it checks the account's owner, that it decodes and validates, and that the builder it records is the payer, leaving only the tier to the coverage check. `CreateFeed` authorized on `FEED_AUTHORITY` or `FOUNDATION` before it read anything else, on the RFC-28 path as much as the old one, which left a mirrored stake buying tier coverage on top of a permission an admin still granted by hand. The stake is tried first and the permission path is untouched, so an admin creating a feed on a builder's behalf works as before and a caller with neither still gets `NotAllowed` from the same call as always. The stake check is deliberately the cheap half: it establishes that the caller is a builder speaking about its own stake, and leaves whether that stake admits the feed to the coverage check that already runs before anything reveals whether the feed exists. So a builder whose tier falls short is told which, rather than told it may not try. `builder` is a caller-supplied argument, so binding it to the payer is what stops one builder's bond creating another's feed. Gated on `allow-staked-feeds` with the rest of RFC-28.
- Collector
  - A failed RIPE Atlas measurement fetch no longer rebuilds the entire measurement fleet. `GetAllMeasurements` returning an error was downgraded to an empty list, so reconciliation read every wanted measurement as missing and every tracked one as unaccounted for: on 2026-09-02 one API error during a restart deleted and recreated all 30 mainnet-beta measurements, the third such wipe in five weeks. The cycle now returns the error and reconciles nothing, which costs an interval without reconciliation instead of a fleet rebuild, fresh RIPE credits and a data gap. Two supporting changes close the other routes into the same state. The state file is written to a temp file and renamed into place, so a kill mid-write can no longer leave a truncated file that reads as "no metadata". And a state file that fails to load holds off both the management and the export cycle, retrying the load each cycle, instead of reconciling against an empty tracker that marks every live measurement for deletion. Export is gated because it ends by saving the state file: with an unloaded tracker it exports nothing and its only effect is to write an empty tracker over the file the next management cycle reads. `Save` refuses to overwrite a file it never loaded, so the invariant also holds for a caller that forgets it. A missing file is still a clean empty state for a first deploy, but only when nothing at all sits at the path and its directory exists: a dangling symlink and an unmounted state directory both surface as "not found" and neither is safe to read as an empty fleet. The management cycle now creates the state directory as export already did, so a not-yet-mounted `--state-dir` is a first deploy rather than a wipe. `Save` renames onto a symlink's target instead of replacing the link, defaults a newly created state file to `0600` rather than inheriting the umask, and fsyncs the parent directory so the rename survives a power loss. The cleanup that removes a measurement with no metadata is deliberately unchanged — it is correct cleanup once the picture is complete. (#4169, #4131)
