...
func TestE2E_DzPrefix_RolloverAllocation(t *testing.T) {
	...
	output, err := dn.Manager.Exec(ctx, []string{"bash", "-c", `
		...
		doublezero device create --code test-dz01 --contributor co01 --facility lax --metro xlax --public-ip "45.33.100.1" --dz-prefixes "45.33.101.0/30,45.33.101.4/30" --mgmt-vrf mgmt --desired-status activated 2>&1
...
func TestE2E_DzPrefix_GrowAndShrink(t *testing.T) {
	...
	output, err := dn.Manager.Exec(ctx, []string{"bash", "-c", `
		set -euo pipefail
		doublezero device create --code test-dz01 --contributor co01 --facility lax --metro xlax --public-ip "45.33.100.1" --dz-prefixes "45.33.101.0/30" --mgmt-vrf mgmt --desired-status activated 2>&1
		doublezero device update --pubkey test-dz01 --max-users 128 2>&1
