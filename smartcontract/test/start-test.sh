...
cp ../programs/dz-sla-program/target/deploy/double_zero_sla_program.so ./bin/double_zero_sla_program.so

#Build the activator
cargo build --manifest-path ../../activator/Cargo.toml
cp ../../activator/target/debug/doublezero-activator ./bin/doublezero-activator


# start the solana test cluster
solana-test-validator --reset --bpf-program ./keypair.json ./bin/double_zero_sla_program.so > ./log/solana.log 2>&1 &
...
doublezero config set --program 7CTniUa88iJKUHTrCkB4TjAoG6TD7AMivhQeuqN2LPtX
...
# Build the activator
./bin/doublezero-activator --program-id 7CTniUa88iJKUHTrCkB4TjAoG6TD7AMivhQeuqN2LPtX  > ./log/activator.log 2>&1 &
...
doublezero device create --code ld4-dz01 --location lhr --exchange xlhr --public-ip "195.219.120.72" --dz-prefixes "195.219.120.72/30,195.219.120.79/30"
...
# create a user
doublezero user create --device ld4-dz01 --client-ip 10.0.0.1
doublezero user create --device ld4-dz01 --client-ip 10.0.0.2
doublezero user create --device ld4-dz01 --client-ip 10.0.0.3
doublezero user create --device ld4-dz01 --client-ip 10.0.0.4
