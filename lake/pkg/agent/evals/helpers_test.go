...
// =============================================================================
// SQL Capturing Querier for validating SQL queries
// =============================================================================

// sqlCapturingQuerier wraps a querier to capture all SQL queries executed.
type sqlCapturingQuerier struct {
	underlying *querier.Querier
	queries    []string
	mu         sync.Mutex
}

func newSQLCapturingQuerier(q *querier.Querier) *sqlCapturingQuerier {
	return &sqlCapturingQuerier{underlying: q}
}

func (q *sqlCapturingQuerier) Query(ctx context.Context, sql string) (querier.QueryResponse, error) {
	q.mu.Lock()
	q.queries = append(q.queries, sql)
	q.mu.Unlock()
	return q.underlying.Query(ctx, sql)
}

func (q *sqlCapturingQuerier) GetQueries() []string {
	q.mu.Lock()
	defer q.mu.Unlock()
	return append([]string{}, q.queries...)
}

// isFactTableQuery checks if SQL references a fact table
func isFactTableQuery(sql string) bool {
	factTables := []string{
	...
	for _, table := range factTables {
		if strings.Contains(sqlLower, table) {
			return true
		}
	}
	return false
...
// validateNoGrafanaMacros checks SQL for Grafana-specific syntax
func validateNoGrafanaMacros(t *testing.T, queries []string) {
	grafanaMacros := []string{
		"$__timeFrom()",
		"$__timeTo()",
		"$__timeFilter",
		"$__interval",
	...
	for _, sql := range queries {
		for _, macro := range grafanaMacros {
			require.NotContains(t, sql, macro,
				"SQL contains Grafana macro %s: %s", macro, truncateForError(sql, 200))
		}
	}
}

// validateFactTableTimeFilters checks that fact table queries have time bounds
func validateFactTableTimeFilters(t *testing.T, queries []string) {
	timePatterns := []string{
		"interval",
		"now()",
		"current_timestamp",
		">= '20", // timestamp literal
		"> '20",
	}

	for _, sql := range queries {
		if !isFactTableQuery(sql) {
			continue
		}
		...
		require.True(t, hasTimeFilter,
			"Fact table query missing time filter: %s", truncateForError(sql, 200))
	}
}

// setupAgentWithCapturingQuerier creates an agent instance with a SQL-capturing querier
func setupAgentWithCapturingQuerier(t *testing.T, ctx context.Context, db duck.DB, capturingQuerier *sqlCapturingQuerier, llmFactory LLMClientFactory, debug bool, debugLevel int, cfg *react.Config) *agent.Agent {
	if cfg == nil {
		cfg = &react.Config{}
	}
	...
	// Create tool client using the capturing querier
	baseToolClient := tools.NewQuerierToolClient(capturingQuerier)
