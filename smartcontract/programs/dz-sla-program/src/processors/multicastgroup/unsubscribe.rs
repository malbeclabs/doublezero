...
pub struct MulticastGroupUnsubscribeArgs {
    pub index: u128,
    pub bump_seed: u8,
...
impl fmt::Debug for MulticastGroupUnsubscribeArgs {
...
pub fn process_unsubscribe_multicastgroup(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    value: &MulticastGroupUnsubscribeArgs,
) -> ProgramResult {
    ...
    #[cfg(test)]
    msg!("process_unsubscribe_multicastgroup({:?})", value);
    ...
    for subscriber in &value.subscribers {
        multicastgroup.subscribers.retain(|&x| x != *subscriber);
    }
    for publisher in &value.publishers {
        multicastgroup.publishers.retain(|&x| x != *publisher);
    }
