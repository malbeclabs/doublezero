...
- CLI
  ...
  - New `access-pass remove-feed --user-payer <pubkey> --feed <name|pubkey>` removes one feed from an EdgeSeat access pass while its other feeds stay active, for example when a trial ends. It unsubscribes every user holding the feed first. It then lowers the multicast user cap to the sum of the remaining seats. The pass's subscriber allowlist is left alone, because feeds grant their groups without it.
- Client
