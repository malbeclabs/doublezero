...
func (d *Devnet) GetOrCreateDeviceOnchain(ctx context.Context, deviceCode string, facility string, metro string, metricsPublisherPK string, publicIP string, prefixes []string, mgmtVrf string) (string, error) {
	...
	if err != nil {
		if errors.Is(err, ErrDeviceNotFoundOnchain) {
			args := []string{"doublezero", "device", "create", "--contributor", "co01", "--code", deviceCode, "--facility", facility, "--metro", metro, "--public-ip", publicIP, "--dz-prefixes", strings.Join(prefixes, ","), "--mgmt-vrf", mgmtVrf}
...
func (d *Devnet) CreateDeviceOnchain(ctx context.Context, deviceCode string, facility string, metro string, publicIP string, prefixes []string, mgmtVrf string) error {
	d.onchainWriteMutex.Lock()
	defer d.onchainWriteMutex.Unlock()

	_, err := d.Manager.Exec(ctx, []string{"doublezero", "device", "create", "--code", deviceCode, "--contributor", "co01", "--facility", facility, "--metro", metro, "--public-ip", publicIP, "--dz-prefixes", strings.Join(prefixes, ","), "--mgmt-vrf", mgmtVrf}, docker.NoPrintOnError())
	if err != nil {
		return fmt.Errorf("failed to create device onchain: %w", err)
	}
