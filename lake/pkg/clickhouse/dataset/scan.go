...
// dereferencePointersToRow converts scanned pointers to a row array.
// Handles both *T (non-nullable) and **T (nullable) types.
func dereferencePointersToRow(valuePtrs []any, columns []string) []any {
	row := make([]any, len(columns))
	for i := range columns {
		// Dereference pointer (handles nullable **T types)
		row[i] = DereferencePointer(valuePtrs[i])
	}
	return row
}
