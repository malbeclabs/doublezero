...
- Serviceability
  ...
  - An EdgeSeat access pass can now hold a user on more than one feed. Joining a group from a second feed on the pass takes that feed's seat, and leaving the feed's last group gives the seat back. One seat per feed, however many of the feed's groups the user joins. Adds `UserDeviceMismatch` (variant 101), returned when the device passed to a role change is not the user's. (#4099)
- Client daemon
