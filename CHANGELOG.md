...
- CI
  ...
  - Testnet release: the generated `DEPLOY.md` now embeds the exact program-deploy commands (keypairs under `~/testnet-ops/`, artifacts from the staged release directory, `doublezero init` to refresh the onchain version) and links the infra deploy runbook, which replaced the Notion doc. Runbook recovery guidance corrected: after a mid-pipeline failure use "Re-run all jobs" — "Re-run failed jobs" leaves previously-skipped downstream jobs skipped and the run can conclude success without running deploys, QA, or announce.
