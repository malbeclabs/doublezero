...
func TestProbing_Worker_SuccessThenFailure_TransitionsAndKernel(t *testing.T) {
	...
	// Per-call gating so we control exactly when a probe is allowed to run/return.
	allow := make(chan struct{}, 4)
	var modeOK atomic.Bool
	modeOK.Store(true) // start with successes
	...
	// Helper to run one deterministic wave: trigger, release exactly 2 probes, wait for drain.
	runWave := func() {
		sched.Trigger()
		allow <- struct{}{}
		allow <- struct{}{}
	...
	// Wave 1: success → still DOWN, OK=1 per route, no kernel adds.
	runWave()
	...
	// Wave 2: success → transition to UP, OK=2 per route, kernel add both.
	runWave()
	require.Eventually(t, func() bool {
	...
	// Wave 3: failure → still UP, Fail=1 per route, kernel still has both.
	runWave()
	require.Eventually(t, func() bool {
	...
	// Wave 4: failure → transition to DOWN, Fail=2 per route, kernel delete both.
	runWave()
	require.Eventually(t, func() bool {
...
func TestProbing_Worker_ListenRetry_UntilContextDone(t *testing.T) {
	...
	cfg := newTestConfig(t, func(c *Config) {
		...
		// very tight retries, but non-zero to avoid timer edge cases on some runners
		c.ListenBackoff = ListenBackoffConfig{Initial: 2 * time.Millisecond, Max: 2 * time.Millisecond, Multiplier: 1}
		c.Limiter, _ = NewSemaphoreLimiter(2)
		c.Scheduler = sched
	})
	...
	// seed a route so stabilized listener actually leads to probes
	...
	w.Start(cfg.Context)
	t.Cleanup(w.Stop)
	require.Eventually(t, func() bool { return w.IsRunning() }, 5*time.Second, 10*time.Millisecond)

	// continuous nudger so a missed Wake pulse can’t strand the loop
	stopNudge := startNudger(t, sched, 5*time.Millisecond)
	t.Cleanup(stopNudge)

	// make sure we’ve actually entered listen loop
	require.Eventually(t, func() bool { return atomic.LoadInt64(&listenCalls) >= 1 }, 5*time.Second, 2*time.Millisecond)

	// now wait for the fourth attempt (three failures + a stable run)
	require.Eventually(t, func() bool { return atomic.LoadInt64(&listenCalls) >= 4 }, 10*time.Second, 2*time.Millisecond)

	// after stabilization, an actual probe should start promptly
	waitEdge(t, probed, 5*time.Second, "probe never started after listener stabilized")
