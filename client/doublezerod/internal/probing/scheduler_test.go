...
func TestProbing_IntervalScheduler_AddPeekPopLeaseAndComplete(t *testing.T) {
	...
	next, ok := s.Peek()
	require.True(t, ok)
	require.Equal(t, 100*time.Millisecond, next.Sub(now))

	later := now.Add(20 * time.Millisecond)
	s.Complete(k2, ProbeOutcome{OK: true, When: later})
	next2, ok := s.Peek()
	require.True(t, ok)
	require.Equal(t, 100*time.Millisecond, next2.Sub(now))
}
...
func TestProbing_IntervalScheduler_LeasePreventsNonstopLoop(t *testing.T) {
	...
	next, ok := s.Peek()
	require.True(t, ok)
	require.Equal(t, 50*time.Millisecond, next.Sub(later))
}
...
func TestProbing_IntervalScheduler_JitterBounds(t *testing.T) {
	...
	_ = s.PopDue(base.Add(10 * time.Second))

	when := base.Add(3 * time.Second)
