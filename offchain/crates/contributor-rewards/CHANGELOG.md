...
- test(contributor-rewards): stop the three keypair tests racing on `REWARDER_KEYPAIR_PATH`. All three set or clear that process-wide variable, and cargo runs tests in parallel threads of one process, so one test clearing it between another's `set_var` and its `load_keypair` made that one fail its `is_ok` assertion. Reproduced at roughly 1 run in 60 before the change and 0 in 60 after. They now carry `#[serial]`; `serial_test` was already a workspace dependency four other crates use, so nothing new is pulled in
