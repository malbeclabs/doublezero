...
func (p *provider) GetCachedCircuitLatencies(ctx context.Context, circuitCode string, epoch uint64, dataProvider string) []stats.CircuitLatencySample {
	p.cacheMu.RLock()
	defer p.cacheMu.RUnlock()
...
func (p *provider) SetCachedCircuitLatencies(ctx context.Context, circuitCode string, epoch uint64, dataProvider string, latencies []stats.CircuitLatencySample, ttl time.Duration) {
	p.cacheMu.Lock()
	defer p.cacheMu.Unlock()
