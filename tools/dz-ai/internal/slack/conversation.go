...
func (m *Manager) cleanup() {
	...
	if len(m.conversations) > maxConversations {
		...
		m.conversations = make(map[string][]agent.Message)
	}
...
type HistoryFetcher interface {
	FetchThreadHistory(ctx context.Context, api *slack.Client, channelID, threadTS, botUserID string) ([]agent.Message, error)
}
...
func (m *Manager) UpdateConversationHistory(threadKey string, msgs []agent.Message) {
	m.conversationsMu.Lock()
	m.conversations[threadKey] = msgs
	m.conversationsMu.Unlock()
}
...
// anthropicMessageAdapter adapts Anthropic messages to agent.Message interface
...
func (f *DefaultFetcher) FetchThreadHistory(ctx context.Context, api *slack.Client, channelID, threadTS, botUserID string) ([]agent.Message, error) {
