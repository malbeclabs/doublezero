...
func TestE2E_DzPrefix_ResourceExhaustion(t *testing.T) {
	...
	if !t.Run("recovery_after_freeing_slot", func(t *testing.T) {
		...
		_, err := freedUser.client.Exec(ctx, []string{"bash", "-c",
			"doublezero user delete --pubkey " + freedUser.pubkey + " --accesspass-type prepaid"})
		require.NoError(t, err)
	...
	t.Run("cleanup_returns_to_baseline", func(t *testing.T) {
		...
		for _, fillUser := range fillUsers[1:] {
			...
			_, err := fillUser.client.Exec(ctx, []string{"bash", "-c",
				"doublezero user delete --pubkey " + fillUser.pubkey + " --accesspass-type prepaid"})
			require.NoError(t, err)
		}
		...
		for _, user := range data.Users {
			if net.IP(user.ClientIp[:]).String() == recoveryClient.CYOANetworkIP {
				...
				_, err := recoveryClient.Exec(ctx, []string{"bash", "-c",
					"doublezero user delete --pubkey " + pubkey + " --accesspass-type prepaid"})
				require.NoError(t, err)
				break
			}
		}
