...
* Multicast publishers should no longer be allocated addresses from the dz_prefix range tied to the device in the serviceability program. In order to accommodate the volume of multicast publishers for some use cases, these address blocks are not large enough (i.e. a /28 or /29 per device). While we chould fill these pools up and spill into a global pool, this address space is routed over the public internet and it's desireable to reserve this range for edge filtering users.
* Initial bootstrapping of a device and enforcing the existance of the two tunnel endpoints will need to be revisited at a later time. 
...
This will be the first generally available feature that doesn't exist on the public internet. There are some expected use cases for multicast but there are likely to be some novel and unanticipated use cases which is, in addition to the anticipated use cases, is a compelling and exciting reason to offer multicast generally. 
