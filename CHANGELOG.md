...
- CLI
  - `doublezero feed create` takes the RFC-28 terms: `--builder`, `--stake-ref`, `--spec-id`, `--sla-hash` and `--committed-rate`. The verb carried code, name, exchange and groups, so the only feed it could make was a catalog one and a builder had no way to deploy without the builder API or a hand-built transaction. The five are refused as a partial set, because a feed carrying some of them is not something the program can make: name one and clap names every other one still missing, rather than the ledger refusing the transaction after a signature. `--committed-rate` takes a bandwidth the way every other bandwidth flag does, or `unmetered` for the tier that is `u64::MAX`, since nobody should have to type that number.
- SDK
  - `CreateFeedCommand` takes `stake_terms`, an optional `FeedStakeTerms` carrying the builder, the stake, the spec, the SLA digest and the committed rate. One value rather than five fields, because the program reads them together: it finds the stake mirror seeded on the stake, checks the tier covers the rate, claims the stake for the feed and starts it Pending. `None` is the catalog feed and the pre-RFC-28 behavior, which is what the defaults on `FeedCreateArgs` already described. A caller that passes terms with a zero builder is refused rather than quietly handed a catalog feed, since that is the value both the program and the instruction builder read as "no stake".
