...
func (d *DimensionType2Dataset) processEmptySnapshot(
	...
	// Build SELECT clause: get latest version of each non-deleted entity from history
	currentSelect := "entity_id, snapshot_ts, ingested_at, op_id, is_deleted, attrs_hash"
	if len(d.pkCols) > 0 {
		currentSelect += ", " + strings.Join(d.pkCols, ", ")
	}
	currentSelect += ", " + d.SelectAttrsExpression("")
