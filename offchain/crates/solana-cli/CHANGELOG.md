...
- `shreds validator-client-rewards set-proportion`: add `--multisig` and `--vault-index`, so a manager held by a Squads vault can set a proportion. The vault stands in for the wallet as manager, no keypair is loaded, and the command prints a base58 payload for import into Squads instead of sending. The payload carries the proportion instruction alone, with no version check and no compute budget, since it executes days after it is written and Squads sets the budget on its own execute transaction, and the output reports the compute units to budget there. Both paths now read the `ValidatorClientRewards` account and refuse a wallet or vault that is not its manager, naming both keys, where a wallet that was not the manager used to fail inside the program with `invalid account data` ([malbeclabs/doublezero#4186](https://github.com/malbeclabs/doublezero/issues/4186))
