...
- Per-tenant unicast path differentiation via `include_topologies: Vec<Pubkey>` on the `Tenant` account — all unicast tenants receive color 2 (UNICAST-DEFAULT) by default; `include_topologies` overrides this to assign specific topologies and steer the tenant onto a designated forwarding plane
...
- **UNICAST-DEFAULT** — The reserved default unicast topology. MUST exist before any link can be activated. The controller resolves it by name via PDA seeds `[b"topology", b"UNICAST-DEFAULT"]` (topology names are uppercased and stored uppercase onchain). It is allocated bit 1 — the lowest allocatable bit, since bit 0 is reserved for UNICAST-DRAINED — giving flex-algo 129 and color 2. Its bit is not otherwise special — the controller uses the name, not the bit, to identify it. Applied to all links at activation. Flex-algo uses `include-any UNICAST-DEFAULT`, so only explicitly tagged links participate in the unicast topology. Untagged links are excluded from unicast but remain available to multicast via IS-IS algo 0.
- **UNICAST-DRAINED** — A controller-managed IS-IS TE admin-group alias used to drain a link from all constrained unicast topologies. It is not a `TopologyInfo` PDA. The controller hardcodes the UNICAST-DRAINED bit as constant `0`; the `AdminGroupBits` allocatable range starts at bit 1 (`IdRange(1, 127)`), so bit 0 is never allocated to a user topology. The controller always defines `administrative-group alias UNICAST-DRAINED group 0` on all devices and always injects `exclude 0` into every `include-any` flex-algo definition. When a link's `unicast_drained` field is `true`, the controller appends `UNICAST-DRAINED` to that interface's `traffic-engineering administrative-group` alongside its permanent topology tags; the permanent tags are not changed. Because `exclude` is evaluated before `include-any` in flex-algo SPF computation (RFC 9350 §5.2.1), the drained link is pruned from all constrained unicast topologies simultaneously. Multicast (algo-0) is unaffected. Drain is a contributor-writable boolean — contributors may drain their own links as a capacity management tool.
...
| Per-tenant unicast path differentiation | ✅ | Architecture proven in lab (BGP color extended communities + `system-colored-tunnel-rib`). All unicast tenants receive color 2 (UNICAST-DEFAULT) by default; `include_topologies` overrides this to steer onto specific topologies |
...
```
admin_group_bit  = next available bit from AdminGroupBits ResourceExtension (1–127; bit 0 reserved for UNICAST-DRAINED)
...
pub struct TopologyInfo {
    pub account_type: AccountType,       // AccountType::Topology
    pub owner: Pubkey,                   // creating foundation key
    pub bump_seed: u8,
    pub name: String,                    // stored uppercase, e.g. "UNICAST-DEFAULT"
    pub admin_group_bit: u8,             // auto-assigned from AdminGroupBits ResourceExtension, 1–127
    pub flex_algo_number: u8,            // auto-assigned, 129–255; always 128 + admin_group_bit
    pub constraint: TopologyConstraint,  // IncludeAny or Exclude
    pub reference_count: u32,            // links referencing this topology; delete blocked while > 0
}
```

PDA seeds: `[b"topology", name.to_uppercase().as_bytes()]` — names are uppercased before seeding and stored uppercase. `TopologyInfo` accounts MUST only be created by foundation keys; all fields are immutable after creation (there is no update instruction). The `reference_count` field is maintained by the program as links are assigned to or cleared from the topology and gates deletion.

Name length MUST NOT exceed 32 bytes, enforced by the program on `create`. Names MUST be ASCII, start with a letter, contain only uppercase letters, digits, and dashes, and are stored uppercase. This keeps PDA seeds well within the 32-byte limit and ensures the admin-group alias name is valid in EOS config.

The program MUST allocate `admin_group_bit` from the range 1–127 (bit 0 is reserved for UNICAST-DRAINED) and MUST return an explicit error if all 127 slots are exhausted. This is a hard constraint: EOS supports bits 0–127 only, and `128 + 127 = 255` is the maximum representable value in `flex_algo_number: u8`.
...
A `link_flags: u32` bitmask field is added to the `Link` account, appended to the serialized layout and defaulting to `0` on existing accounts. The drain state is stored in bit 0, `LINK_FLAG_UNICAST_DRAINED = 0x01`, and surfaced as the `unicast_drained` boolean at the instruction and CLI layer. The flag is contributor-writable — a contributor may drain their own link as a capacity management tool. When set, the controller appends `UNICAST-DRAINED` to the interface's admin-group config alongside the permanent topology tags. It is set independently of `link.status` — a link can be `soft-drained` and unicast-drained simultaneously; the two are orthogonal.

```rust
// Contributor-writable (stored as bit 0 of link_flags)
if let Some(unicast_drained) = value.unicast_drained {
    if unicast_drained {
        link.link_flags |= LINK_FLAG_UNICAST_DRAINED;
    } else {
        link.link_flags &= !LINK_FLAG_UNICAST_DRAINED;
    }
}
...
doublezero link topology assign-node-segments --name <NAME>
doublezero link topology list
```

- `create` — creates a `TopologyInfo` PDA; allocates the lowest available admin-group bit from the `AdminGroupBits` `ResourceExtension`; derives and stores flex-algo number; stores the specified constraint (`include-any` or `exclude`). All fields are immutable after creation. MUST fail if the name already exists. The `AdminGroupBits` allocatable range starts at bit 1 (`IdRange(1, 127)`), reserving bit 0 for UNICAST-DRAINED so it is never allocated to a user topology. `create` does **not** backfill node segments onto existing loopbacks — run `assign-node-segments` (or `doublezero migrate flex-algo`) afterward. Device impact is controlled entirely by the controller feature config — no device config is generated until `flex_algo.enabled: true` is set in the config file.
...
- `assign-node-segments` — allocates a `FlexAlgoNodeSegment` for this topology on every Vpnv4 loopback interface that does not yet have one, drawing each index from the `SegmentRoutingIds` `ResourceExtension`. This is the sweep that makes every device advertise a node-SID for the topology; it MUST be run after `create` for any loopbacks that existed before the topology was created (loopbacks activated afterward get a segment at interface activation). Idempotent — only interfaces missing an entry are updated.
...
UNICAST-DEFAULT    include-any   129         1                 2           link-xyz789
```
...
An `include_topologies: Vec<Pubkey>` field is added to the serviceability `Tenant` account. Each entry holds the pubkey of a `TopologyInfo` PDA. All unicast tenants receive color 2 (UNICAST-DEFAULT) by default; setting `include_topologies` overrides this to assign specific topologies based on business requirements. The field appends to the end of the serialized layout, defaulting to an empty vector on existing accounts.
...
- `--include-topologies default` sets `include_topologies` to an empty vector, reverting the tenant to the default color 2 (UNICAST-DEFAULT).
...
| UNICAST-DRAINED | system constant | 0 | — (no flex-algo) | — | Exclude-only; injected into all include-any definitions |
| UNICAST-DEFAULT | include-any | 1 | 129 | 2 | Only UNICAST-DEFAULT tagged links |

The flex-algo definition MUST be configured on each DZD by the controller. The `color` field MUST be included and set to `admin_group_bit + 1`. Every `include-any` flex-algo definition MUST include `exclude 0` (the UNICAST-DRAINED bit) — this is unconditional and not dependent on any link being drained. Using UNICAST-DEFAULT as an example:
...
Flex-algo 129 (`UNICAST-DEFAULT`) computes an IS-IS SPF over only those links tagged `UNICAST-DEFAULT`. The `color 2` field causes EOS to install these tunnels in `system-colored-tunnel-rib` keyed by (endpoint, 2). Devices that participate in flex-algo 129 advertise both an algo-0 node-segment and an algo-129 node-segment via their loopback.
...
|---|---|---|
| VPN unicast | flex-algo 129 (`system-colored-tunnel-rib`, color 2) | Tagged links only |
...
- If `tenant.include_topologies` is non-empty, resolve each `TopologyInfo` PDA and compute `AdminGroupBit + 1` for each. All resolved color values are stamped in a single `set extcommunity color` statement (e.g., `set extcommunity color 2 color 3`).
- If `tenant.include_topologies` is empty, use the default unicast color: resolve the UNICAST-DEFAULT `TopologyInfo` by name (`[b"topology", b"UNICAST-DEFAULT"]`) and compute its color as `admin_group_bit + 1` (color 2).
...
`$.AllTopologies` is the ordered list of all `TopologyInfo` accounts, sorted by `AdminGroupBit`. `.Color` is computed as `AdminGroupBit + 1`. `UNICAST-DRAINED group 0` is a system constant — always rendered, not derived from any `TopologyInfo` PDA. The flex-algo name (e.g., `UNICAST-DEFAULT`) is the topology name stored uppercase in `TopologyInfo` and rendered uppercase in EOS. An `exclude`-constraint topology renders `administrative-group exclude <bit>,0` (still excluding the drain bit); an `include-any` topology renders `include any <bit> exclude 0`.
...
**Topology creation backfill:** When a new `TopologyInfo` account is created, a `FlexAlgoNodeSegment` entry MUST be allocated on every existing `Interface` account with `loopback_type = Vpnv4`. This is **not** done automatically by the creation processor — allocating across every device interface exceeds what a single instruction can touch. Instead it is a separate sweep, exposed as `doublezero link topology assign-node-segments --name <NAME>` (and run as part of `doublezero migrate flex-algo`), which allocates a segment index from `SegmentRoutingIds` for each loopback still missing one. The operator MUST run this after creating a topology. Loopbacks created after the topology exists are allocated a segment at interface activation. Once the sweep completes, the controller picks up the new entries on its next cycle and pushes the updated `node-segment` config to all devices, so all devices advertise a node-SID for every defined topology, maintaining the universal participation requirement.
...
`TopologyInfo` was added to the Go and Python SDKs, and the `link` (`link_topologies`, `link_flags`) and `tenant` (`include_topologies`) deserialization structs were extended in both. Fixtures were regenerated. **The TypeScript SDK was not updated** and still lacks `TopologyInfo` and the new `Link`/`Tenant` fields — this remains outstanding. Because the new fields are appended to the serialized layout, the TypeScript deserializers continue to work for all pre-existing fields; they simply do not expose the flex-algo additions.
...
- A foundation key MUST be able to create a `TopologyInfo` account; admin-group bit MUST be allocated from the `AdminGroupBits` `ResourceExtension`. The first topology will receive bit 1 (bit 0 is reserved for UNICAST-DRAINED) and flex-algo number 129.
- Creating a second topology MUST allocate bit 2 — bit 0 is never allocated to a user topology (the allocatable range is 1–127).
...
- Setting `include_topologies` to an empty vector MUST be accepted and revert the tenant to the default color 2 (UNICAST-DEFAULT).
...
- The `router traffic-engineering` block MUST always include `administrative-group alias UNICAST-DRAINED group 0` when `enabled: true`, regardless of whether any link is drained.
- The `router traffic-engineering` block MUST include `color <admin_group_bit + 1>` on each flex-algo definition.
- Each `include-any` flex-algo definition MUST include `exclude 0` unconditionally. An `exclude`-constraint topology renders `exclude <bit>,0`.
...
- A per-tunnel inbound route-map MUST include `set extcommunity color 2` for a unicast tenant with empty `include_topologies` when the tenant or device is in the `community_stamping` config and `.LinkTopologies` is non-empty.
- A per-tunnel inbound route-map MUST include `set extcommunity color 2 color 3` for a unicast tenant with `include_topologies` referencing two `TopologyInfo` accounts (bits 1 and 2 — bit 0 is reserved for UNICAST-DRAINED and never allocated).
...
- **Topology creation**: After a foundation key creates a `TopologyInfo` for "UNICAST-DEFAULT" (constraint include-any) and `enabled: true` is set, the controller MUST push `router traffic-engineering` config including `administrative-group alias UNICAST-DRAINED group 0`, `administrative-group alias UNICAST-DEFAULT group <bit>`, the corresponding `flex-algo` definition with `include any <bit> exclude 0 color <bit+1>`, and the BGP `next-hop resolution ribs` line to all devices.
...
- **Flex-algo topology**: With links tagged UNICAST-DEFAULT, `show isis flex-algo` on participating devices MUST show algo 129 including only UNICAST-DEFAULT links. Untagged links MUST be absent from the algo-129 LSDB view.
- **Colored tunnel RIB**: `show tunnel rib system-colored-tunnel-rib brief` MUST show (endpoint, color 2) entries for each participating device, resolving via UNICAST-DEFAULT tunnels.
- **VPN unicast path selection**: A BGP VPN-IPv4 route carrying `Color:CO(00):2` MUST resolve its next-hop through the color-2 (UNICAST-DEFAULT) tunnel in `system-colored-tunnel-rib`, traversing only UNICAST-DEFAULT tagged links.
- **Per-tenant topology — single**: A tenant with `include_topologies = [SHELBY pubkey]` MUST have `Color:CO(00):3` stamped on its inbound routes (with UNICAST-DEFAULT at bit 1 and SHELBY at bit 2, SHELBY has color 3). A tenant with empty `include_topologies` (default) MUST have `Color:CO(00):2` (UNICAST-DEFAULT).
- **Per-tenant topology — multi**: A tenant with `include_topologies = [UNICAST-DEFAULT pubkey, SHELBY pubkey]` MUST have both `Color:CO(00):2` and `Color:CO(00):3` stamped. `show ip route vrf <tenant>` MUST show the lower-metric color tunnel selected for next-hop resolution.
...
- **Drain exclude precedence**: The `exclude 0` clause in each `include-any` flex-algo definition MUST take precedence over `include any <topology-bit>` — a link with `unicast_drained = true` MUST NOT appear in any constrained unicast topology's SPF regardless of its permanent tags (RFC 9350 §5.2.1).
- **UNICAST-DRAINED alias always present**: With `enabled: true` and no links drained, `show traffic-engineering` MUST show `administrative-group alias UNICAST-DRAINED group 0` defined on all devices. The alias and `exclude 0` clauses are structural, not conditional on drain state.
...
```
MUST confirm algo 129 is advertised at Level-2, the `color` field is set, and the selected path includes only UNICAST-DEFAULT tagged links.
...
```
MUST confirm (endpoint, color 2) entries are present for each participating device, resolving via UNICAST-DEFAULT tunnels.
...
```
MUST confirm BGP VPN-IPv4 routes carry `Color:CO(00):2` and resolve next-hops through `system-colored-tunnel-rib` tunnels.
...
- **serviceability** — new `TopologyInfo` PDA (foundation-managed, one per topology; carries a `reference_count` that blocks deletion while links reference it); new `AdminGroupBits` `ResourceExtension` account for persistent bit allocation (allocatable range 1–127; bit 0 reserved for UNICAST-DRAINED, so user topologies start at bit 1); new `link_topologies: Vec<Pubkey>` field (cap 8) on `Link` with foundation-only write restriction; new `link_flags: u32` bitmask on `Link` with `LINK_FLAG_UNICAST_DRAINED = 0x01` (contributor-writable, defaults to `0`, surfaced as the `unicast_drained` boolean); new `flex_algo_node_segments: Vec<FlexAlgoNodeSegment>` field on `Interface` (one entry per `TopologyInfo` topology, each with its own allocated node segment index); new `include_topologies: Vec<Pubkey>` field on `Tenant` with foundation-only write restriction.
- **controller** — new `-features-config` flag and `features.yaml` config file; reads `link.link_topologies[0]` and `link.unicast_drained`, resolves `TopologyInfo` PDAs, generates IS-IS TE admin-group config on interfaces (appending `UNICAST-DRAINED` when `unicast_drained = true`, respecting `link_tagging.exclude.links`), always defines `administrative-group alias UNICAST-DRAINED group 0` and injects `exclude 0` into every `include-any` flex-algo definition, `system-colored-tunnel-rib` BGP resolution profile, and adds `set extcommunity color` to the existing per-tunnel inbound route-maps (`RM-USER-{{ .Id }}-IN`) for stamping-eligible tunnels; generates `no` commands for full revert when `enabled: false`.
- **CLI** — topology lifecycle commands (`create`, `delete`, `clear`, `assign-node-segments`, `list`) under `doublezero link topology` (topology fields are immutable, so there is no `update`); `link update` gains `--link-topology` and `--unicast-drained true|false`; `link get` / `link list` display both fields including derived color; `link topology list` warns on disconnected topologies. A hidden `doublezero migrate flex-algo [--dry-run]` command backfills pre-existing links and loopbacks.
- **SDKs** — `TopologyInfo` and the new `Link` (`link_topologies`, `link_flags`) and `Tenant` (`include_topologies`) fields added to the Go and Python SDKs; the TypeScript SDK is not yet updated (outstanding).
...
- Adding a new topology MUST NOT require a code change or deploy — DZF creates the `TopologyInfo` account via the CLI, then runs `doublezero link topology assign-node-segments` (or `doublezero migrate flex-algo`) to allocate `FlexAlgoNodeSegment` entries on all existing Vpnv4 loopbacks, and the controller picks up the new topology and updated loopback entries on the next reconciliation cycle. The node-segment sweep is a separate step, not automatic at topology creation.

### Testing

| Layer | Tests |
|---|---|
| Smart contract | `TopologyInfo` create/update/delete lifecycle; `AdminGroupBits` `ResourceExtension` allocation and no-reuse after deletion; bit 0 reserved for UNICAST-DRAINED (allocatable range 1–127); foundation-only authorization; `link_topologies` assignment and clearing; delete blocked (via `reference_count`) when links assigned; `clear` removes topology from all links; `link_topologies` cap at 8 enforced; `include_topologies` assignment on tenant; `unicast_drained` (`link_flags` bit 0) contributor-writable set/unset on Link; `flex_algo_node_segments` allocated at loopback activation and deallocated on `remove`; `assign-node-segments` sweep allocates `FlexAlgoNodeSegment` on all existing Vpnv4 loopbacks (not automatic at topology creation) |
| Controller (unit) | Interface config with and without topology; link tagging exclude list respected; remove/add diff on topology change; `router traffic-engineering` block with `color` field; UNICAST-DRAINED alias (group 0) and `exclude 0` always present in all `include-any` definitions regardless of drain state; `unicast_drained: true` appends UNICAST-DRAINED to interface admin-group; `unicast_drained: false` removes UNICAST-DRAINED from interface admin-group; loopback node-segment lines rendered correctly — one per topology from `flex_algo_node_segments`; `system-colored-tunnel-rib` BGP resolution config; single-topology and multi-topology per-tenant stamping; community stamping per-tenant and per-device; stamping exclude respected; startup check blocks `enabled: true` when any Vpnv4 loopback has empty `flex_algo_node_segments`; full revert on `enabled: false`; new topology triggers config push to all devices |
| SDK (unit) | `TopologyInfo` Borsh round-trip; `link_topologies` and `unicast_drained` fields in `link get` / `link list` output; `include_topologies` field in `tenant get` / `tenant list` output (Go and Python; TypeScript not yet updated); color displayed correctly |
| End-to-end (cEOS) | Topology create → controller config push verified; admin-group applied and removed on interface; UNICAST-DRAINED alias and `exclude 0` verified on all devices; unicast-drained link excluded from all constrained topologies but present in algo-0; link tagging exclude list verified; flex-algo 129 topology includes only UNICAST-DEFAULT links; `system-colored-tunnel-rib` populated; VPN unicast resolves via color-2 tunnel; per-tenant single and multi-topology stamping verified; topology fallback verified; community stamping per-device verified; stamping exclude verified; multicast RPF unchanged; new topology creation → node-segment lines appear on all devices; topology clear removes admin-groups; full revert on `enabled: false` verified |
...
Introducing multiple forwarding topologies over the same physical links has implications for the network visualization tools in the lake repository. Today, lake displays a single IS-IS topology where every link is treated equally. With flex-algo, a link's effective participation depends on its topology assignment and the traffic type being considered — an untagged link is present in the algo-0 view but absent from the algo-129 (UNICAST-DEFAULT) view used by VPN unicast.

Areas that SHOULD evolve:

- **Topology view filtering** — the topology map SHOULD allow operators to switch between views: all links (algo 0), unicast topology (algo 129), or a per-service overlay. A link's topology assignment SHOULD be visually distinct and its inclusion or exclusion from each forwarding plane SHOULD be clear.
...
_Resolved during implementation: DZF adopted functional, uppercase names (e.g. `UNICAST-DEFAULT`, `SHELBY`). Topology names are uppercased and stored uppercase onchain, and appear verbatim as the EOS admin-group alias and flex-algo name._

- **Topology naming convention** (resolved — see above): The RFC defines `unicast-default` and `shelby` as the initial topologies. Should DZF adopt a consistent naming convention for future topologies? Options include:
