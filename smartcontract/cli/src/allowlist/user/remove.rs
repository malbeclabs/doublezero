...
pub struct RemoveUserAllowlistCliCommand {
    /// User public key to remove from the allowlist
    #[arg(long)]
    pub pubkey: String,
}
...
mod tests {
    use crate::{
        allowlist::user::remove::{RemoveUserAllowlistCliCommand, RemoveUserAllowlistCommand},
        requirements::{CHECK_BALANCE, CHECK_ID_JSON},
    ...
    fn test_cli_user_allowlist_remove() {
        ...
        client
            ...
            .returning(|_| Ok(()));
        client
            .expect_remove_user_allowlist()
            .with(predicate::eq(RemoveUserAllowlistCommand { pubkey }))
            .returning(move |_| Ok(signature));

        /*****************************************************************************************************/
        let mut output = Vec::new();
        let res = RemoveUserAllowlistCliCommand {
