...
- SDK
  ...
  - The Rust SDK (`smartcontract/sdk/rs`) behind the `doublezero` CLI now retries a transient failure carried inside a decoded JSON-RPC error envelope, not just one that surfaced at transport level. `is_retryable_rpc_error` previously matched only `Io`/`Reqwest`/`Middleware`, so an `RpcResponseError` holding an HTTP 429/5xx that a provider LB put in the envelope `code`, a `-32005`/`-32004`/`-32429` "busy, retry later" node code, or transient wording like "service unavailable" with no machine-readable code (the Helius `-32603` shape) was a one-shot failure for every CLI read. Rust rode out the 2026-07-28 incident only because those 503s happened to arrive as `Reqwest` errors; had they arrived as decoded envelopes — as they did for Go — every CLI invocation would have failed on the first attempt. Classification now mirrors the Go fix from #4100 (`tools/solana/pkg/jsonrpc`). Onchain and request-level rejections (`-32002` preflight, `-32602`, `-32601`), `-32003` signature-verification failure and `-32011` history-not-available stay non-retryable, and no `send_transaction` path is wrapped with this predicate, so an accepted-but-unacknowledged send is never resent. (#4101)
- CI
