...
func TestE2E_UserBan(t *testing.T) {
	...
	_, err = dn.Manager.Exec(t.Context(), []string{"bash", "-c", "doublezero link create wan --code \"la2-dz01:ny5-dz01\" --contributor co01 --side-a la2-dz01 --side-a-interface Ethernet2 --side-z ny5-dz01 --side-z-interface Ethernet2 --bandwidth \"10 Gbps\" --delay-ms 40 --jitter-ms 3 --desired-status activated"})
	require.NoError(t, err)
...
func runUserBanIBRLWorkflowTest(t *testing.T, log *slog.Logger, client1 *devnet.Client, client2 *devnet.Client, client3 *devnet.Client, dn *devnet.Devnet, deviceCode1 string, deviceCode2 string) {
	...
	_, err = dn.Manager.Exec(t.Context(), []string{"doublezero", "user", "delete", "--pubkey", user_pk, "--accesspass-type", "prepaid"})
	require.NoError(t, err)
