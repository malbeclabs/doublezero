...
- CLI
  ...
  - New `access-pass remove-feed --user-payer <pubkey> --feed <name|pubkey>` removes one feed from an EdgeSeat access pass while its other feeds stay active, for example when a trial ends. It prints the plan and asks for confirmation, and `--dry-run` prints the plan without sending anything. It drops the feed's seat first, so no new user can subscribe to the feed. It then unsubscribes every user holding the feed and deletes each user left with no feed. Last, it lowers the multicast user cap to the sum of the remaining seats. The pass's subscriber allowlist is left alone, because feeds grant their groups without it.
- Client
