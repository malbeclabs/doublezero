...
func (v *View) Refresh(ctx context.Context) error {
	...
	defer func() {
		...
		mcpmetrics.ViewRefreshDuration.WithLabelValues("telemetry-usage").Observe(duration.Seconds())
		if err := recover(); err != nil {
			mcpmetrics.ViewRefreshTotal.WithLabelValues("telemetry-usage", "error").Inc()
			panic(err)
		}
	}()
	...
	if len(usage) == 0 {
		...
		mcpmetrics.ViewRefreshTotal.WithLabelValues("telemetry-usage", "success").Inc()
		return nil
	}
...
func (v *View) Close() error {
	if v.cfg.InfluxDB != nil {
		err := v.cfg.InfluxDB.Close()
		// Ignore connection closing errors during shutdown - they're expected
		if err != nil {
			errStr := err.Error()
			if strings.Contains(errStr, "connection is closing") ||
				strings.Contains(errStr, "code = Canceled") ||
				strings.Contains(errStr, "grpc: the client connection is closing") {
				return nil // Expected during shutdown, don't log as error
			}
		}
		return err
	}
