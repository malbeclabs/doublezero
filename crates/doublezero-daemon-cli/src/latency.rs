//! `doublezero latency` — show per-device latency, plus the device-selection
...
use crate::{
    client::{DaemonClient, LatencyRecord},
    helpers,
    ledger::LedgerClient,
...
pub fn get_device_tunnel_endpoints(device: &Device) -> Vec<Ipv4Addr> {
...
pub async fn retrieve_latencies<D: DaemonClient>(
    daemon: &D,
    ...
    let mut latencies = loop {
        let response = daemon.latency().await.map_err(|e| eyre::eyre!(e))?;
...
///
/// # Arguments
/// * `daemon` - Daemon client for fetching latency data
...
pub async fn best_latency<D: DaemonClient>(
    daemon: &D,
    devices: &HashMap<Pubkey, Device>,
    ...
    let mut latencies = retrieve_latencies(daemon, devices, true, spinner).await?;
...
mod tests {
    use super::*;
    use crate::{
        client::{LatencyResponse, MockDaemonClient},
    ...
    fn make_device(status: DeviceStatus, users_count: u16) -> (Pubkey, Device) {
        make_device_with_ip(status, users_count, Ipv4Addr::UNSPECIFIED, vec![])
    }
    ...
    #[test]
    fn test_retrieve_latencies_filters_and_sorts() {
        block_on(async {
            ...
            daemon.expect_latency().returning(move || {
                Ok(LatencyResponse {
                    ready: true,
            ...
            let result = retrieve_latencies(&daemon, &devices, true, None)
                .await
                .unwrap();
            ...
            let result = retrieve_latencies(&daemon, &devices, false, None)
                .await
                .unwrap();
    ...
    #[test]
    fn test_best_latency_prefers_current_within_tolerance() {
        block_on(async {
            let (pk1, dev1) = make_device(DeviceStatus::Activated, 0);
            let (pk2, dev2) = make_device(DeviceStatus::Activated, 0);
            ...
            let result = best_latency(&daemon, &devices, true, None, Some(&pk2), &[])
                .await
                .unwrap();
    ...
    #[test]
    fn test_best_latency_selects_lowest() {
        block_on(async {
            let (pk1, dev1) = make_device(DeviceStatus::Activated, 0);
            let (pk2, dev2) = make_device(DeviceStatus::Activated, 0);
            let (pk3, dev3) = make_device(DeviceStatus::Activated, 0);
            ...
            let result = best_latency(&daemon, &devices, true, None, None, &[])
                .await
                .unwrap();
    ...
    #[test]
    fn test_best_latency_ignores_unreachable_devices() {
        block_on(async {
            let (pk1, dev1) = make_device(DeviceStatus::Activated, 0);
            let (pk2, dev2) = make_device(DeviceStatus::Activated, 0);
            let (pk3, dev3) = make_device(DeviceStatus::Activated, 0);
    ...
    #[test]
    fn test_best_latency_ignores_faster_devices_at_max_users() {
        block_on(async {
    ...
    #[test]
    fn test_best_latency_current_faster_but_at_max_users() {
        block_on(async {
            let (pk1, dev1) = make_device(DeviceStatus::Activated, 0);
    ...
    #[test]
    fn test_best_latency_excludes_ips() {
        block_on(async {
            ...
            let result = best_latency(&daemon, &devices, true, None, None, &[ip1, ip2, ip3]).await;
    ...
    #[test]
    fn test_best_latency_excludes_specific_ip() {
        block_on(async {
            let ip1: Ipv4Addr = "10.0.0.1".parse().unwrap();
            let ip2: Ipv4Addr = "10.0.0.2".parse().unwrap();
            let (pk1, dev1) = make_device_with_ip(DeviceStatus::Activated, 0, ip1, vec![]);
            ...
            let result = best_latency(&daemon, &devices, true, None, None, &[excluded_ip])
                .await
                .unwrap();
    ...
    #[test]
    fn test_best_latency_device_with_multiple_endpoints_not_excluded() {
        block_on(async {
            ...
            let ip2: Ipv4Addr = "10.0.0.2".parse().unwrap();
            let (pk1, dev1) =
                make_device_with_ip(DeviceStatus::Activated, 0, ip1, vec![ip1_tunnel]);
            let (pk2, dev2) = make_device_with_ip(DeviceStatus::Activated, 0, ip2, vec![]);
    ...
    #[test]
    fn test_best_latency_device_all_endpoints_excluded() {
        block_on(async {
            ...
            let result = best_latency(&daemon, &devices, true, None, None, &[ip1, ip1_tunnel])
                .await
                .unwrap();
    ...
    #[test]
    fn test_best_latency_prefers_same_device_with_available_endpoint() {
        block_on(async {
            ...
            let result = best_latency(&daemon, &devices, true, None, Some(&pk1), &[ip1])
                .await
                .unwrap();
    ...
    #[test]
    fn test_retrieve_latencies_sorts_by_min_not_avg() {
        block_on(async {
            let (pk1, dev1) = make_device(DeviceStatus::Activated, 0);
            let (pk2, dev2) = make_device(DeviceStatus::Activated, 0);
    ...
    #[test]
    fn test_retrieve_latencies_tiebreaker_prefers_lower_avg() {
        block_on(async {
            let (pk1, dev1) = make_device(DeviceStatus::Activated, 0);
            let (pk2, dev2) = make_device(DeviceStatus::Activated, 0);
    ...
    #[test]
    fn test_best_latency_selects_by_min_not_avg() {
        block_on(async {
            let (pk1, dev1) = make_device(DeviceStatus::Activated, 0);
            let (pk2, dev2) = make_device(DeviceStatus::Activated, 0);
    ...
    #[test]
    fn test_best_latency_current_device_seeded_by_min_not_avg() {
        block_on(async {
            // Verify that when a current_device is set, the tolerance window is seeded from
            // min_latency_ns, not avg_latency_ns.
            //
    ...
    #[test]
    fn test_retrieve_latencies_waits_for_daemon_ready() {
        block_on(async {
            let (pk1, dev1) = make_device(DeviceStatus::Activated, 0);
            let mut devices = HashMap::new();
            devices.insert(pk1, dev1);
    ...
    #[test]
    fn test_retrieve_latencies_ready_but_empty_returns_error() {
        block_on(async {
            ...
            let result = retrieve_latencies(&daemon, &devices, false, None).await;
            assert!(result.is_err());
            assert_eq!(
    ...
    #[test]
    fn test_retrieve_latencies_reachable_only_error_distinguishes_cause() {
        block_on(async {
            let (pk1, dev1) = make_device(DeviceStatus::Activated, 0);
            let mut devices = HashMap::new();
            devices.insert(pk1, dev1);
            ...
            let result = retrieve_latencies(&daemon, &devices, true, None).await;
            assert!(result.is_err());
            assert_eq!(
                result.unwrap_err().to_string(),
    ...
    #[test]
    fn test_latency_verb_json_output() {
        block_on(async {
            let (pk1, dev1) = make_device(DeviceStatus::Activated, 0);
            let mut devices = HashMap::new();
            devices.insert(pk1, dev1);
            ...
            ledger
                .expect_list_device()
                .returning(move || Ok(device_map.clone()));

            let ctx = cli_context_default_for_tests();
            let mut out = Vec::new();
            let result = Latency { json: true }
                .execute(&ctx, &daemon, &ledger, &mut out)
                .await;

            assert!(result.is_ok());
            let output = String::from_utf8(out).unwrap();
            let parsed: Vec<LatencyRecord> = serde_json::from_str(output.trim()).unwrap();
            assert_eq!(parsed.len(), 1);
            assert_eq!(parsed[0].device_pk, pk1.to_string());
            assert_eq!(parsed[0].min_latency_ns, 5_000_000);
        });
    }

    #[test]
    fn test_latency_verb_daemon_not_running() {
        block_on(async {
            let mut daemon = MockDaemonClient::new();
