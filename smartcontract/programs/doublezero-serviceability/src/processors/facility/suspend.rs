...
pub struct FacilitySuspendArgs {}

impl fmt::Debug for FacilitySuspendArgs {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "")
    }
}

pub fn process_suspend_facility(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    _value: &FacilitySuspendArgs,
) -> ProgramResult {
    ...
    #[cfg(test)]
    msg!("process_suspend_facility({:?})", _value);
    ...
    if facility.status != FacilityStatus::Activated {
        #[cfg(test)]
        msg!("{:?}", facility);
