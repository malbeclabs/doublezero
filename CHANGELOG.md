...
- Serviceability
  ...
  - EdgeSeat access passes admit multi-feed users. `UpdateMulticastGroupRoles` now runs the feed metro gate on a role add, ticking the covering feed's seat and appending it to `user.feed_pks`, so a user joining groups drawn from several feeds serving its metro holds one seat per feed. Previously the post-activation path validated coverage without ticking, so every feed past the one used at connect was consumed for free. A seat is held per feed rather than per group (a second group inside a feed the user already holds does not tick again), and a removal releases the seat once the user's last group in that feed goes away. The device and feed accounts are required to add a role on an EdgeSeat pass and optional to remove one, so the removal-only cleanup preceding `DeleteUser`/`RequestBanUser` is unaffected and its seat stays held until delete. New `UserDeviceMismatch` error (variant 101) replaces a bare `InvalidAccountData` when the supplied device is not the user's. (#4099)
- CLI
  - `user subscribe` and `user create-subscribe` accept `--feed <PUBKEY|CODE>` for EdgeSeat access passes, which are admitted by their feeds rather than by the multicast-group allowlists; without it neither verb could drive such a pass at all. The flag is an override: the SDK otherwise resolves the user's device and the pass's feed serving that metro and carrying the group. A feed code resolves against the device's metro, since the same code can exist in more than one. (#4099)
- Device controller
...
- SDK
  ...
  - `UpdateMulticastGroupRolesCommand` no longer checks EdgeSeat role changes against the multicast-group allowlists, which EdgeSeat passes bypass onchain: the check rejected every such change before the transaction was even built. It now resolves the EdgeSeat metro gate itself (the user's device, plus the pass's feed serving that metro and carrying the target group) and hands it to `update_multicast_group_roles`, which takes the device/feed pair as a new optional argument. (#4099)
