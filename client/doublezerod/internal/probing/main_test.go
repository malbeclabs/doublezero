...
func (s *fakeScheduler) Trigger() {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.wavePending { // already pending: just wake; don't reset waveDone
		s.signalLocked()
		return
	}
	s.wavePending = true
...
// Wait until the current wave is drained (all due items completed) or timeout.
func (s *fakeScheduler) waitDrained(t *testing.T, timeout time.Duration) {
	s.mu.Lock()
	ch := s.waveDone
	s.mu.Unlock()
	if ch == nil {
		// No wave in progress; treat as drained.
		return
	}
	select {
	case <-ch:
	case <-time.After(timeout):
		t.Fatalf("scheduler did not drain in time")
	}
}
...
func (s *fakeScheduler) PopDue(_ time.Time) []RouteKey {
	...
	s.waveOut = len(out)
	// If nothing to do this wave, close immediately so waiters don’t hang.
	if s.waveOut == 0 && s.waveDone != nil {
		close(s.waveDone)
		s.waveDone = nil
	}
