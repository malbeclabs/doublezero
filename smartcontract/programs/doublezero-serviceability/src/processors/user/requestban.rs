...
pub struct UserRequestBanArgs {
    ...
    /// RequestBanUser always deallocates resources and moves the user to Banned.
...
pub fn process_request_ban_user(
    ...
    if value.dz_prefix_count == 0 {
        #[cfg(test)]
        msg!("dz_prefix_count must be > 0; RequestBanUser requires on-chain deallocation");
    ...
    // Atomic deallocate + move user to Banned.
    ...
    #[cfg(test)]
    msg!("RequestBanUser: User resources deallocated, status = Banned");
