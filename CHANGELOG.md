...
- Collector
  - The Wheresitup collector drops tracked jobs whose results WheresItUp has already discarded instead of polling them, asks for a one-hour expiry rather than ten minutes, and reports what it dropped. Past `expire_after` the API keeps returning the job with an empty `complete` and a populated `in_progress`, the same shape as a job still running, so an expired job was polled on every cycle for two hours and yielded nothing. A vendor-side fault left thousands of jobs stuck; polling them stretched the export pass and the collection cycle past ten minutes, so every newly created job was first polled after its own expiry, read as in progress, and was itself retained. That loop sustained itself after the vendor recovered and left mainnet-beta's wheresitup provider reporting no samples for about 22 hours, with nothing alerting: the missing-samples counter only moved on cycles that exported at least one record, so the cycles that exported none reported nothing missing. The one-hour expiry — which job creation already claimed in its logs while the payload sent ten minutes — keeps a job's results in place until the next cycle polls them, and pairing the state-file cutoff with it halves the steady-state poll set to about eleven batches at the six-minute cadence. Missing samples are now counted on every cycle as the RIPE Atlas collector already does, dropped jobs increment a counter, and only the newest sample per circuit is exported per pass, since downstream derives each sample's timestamp from its position in the account.
