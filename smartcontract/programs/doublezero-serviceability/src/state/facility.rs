...
impl From<u8> for FacilityStatus {
    fn from(value: u8) -> Self {
        match value {
            0 => FacilityStatus::Pending,
            1 => FacilityStatus::Activated,
            2 => FacilityStatus::Suspended,
            _ => FacilityStatus::Pending,
        }
    }
}
...
impl fmt::Display for FacilityStatus {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FacilityStatus::Pending => write!(f, "pending"),
            FacilityStatus::Activated => write!(f, "activated"),
            FacilityStatus::Suspended => write!(f, "suspended"),
        }
    }
}
...
impl TryFrom<&[u8]> for Facility {
    ...
    fn try_from(mut data: &[u8]) -> Result<Self, Self::Error> {
        ...
        if out.account_type != AccountType::Facility {
...
impl TryFrom<&AccountInfo<'_>> for Facility {
    type Error = ProgramError;
    ...
    fn try_from(account: &AccountInfo) -> Result<Self, Self::Error> {
        ...
        if res.is_err() {
            msg!("Failed to deserialize Facility: {:?}", res.as_ref().err());
        }
        res
    }
}

impl Validate for Facility {
    fn validate(&self) -> Result<(), DoubleZeroError> {
        // Account type must be Facility
        if self.account_type != AccountType::Facility {
...
mod tests {
    ...
    #[test]
    fn test_state_compatibility_facility() {
        ...
        crate::helper::base_tests::test_parsing::<Facility>(&versions).unwrap();
    }

    #[test]
    fn test_state_facility_try_from_defaults() {
        let data = [AccountType::Facility as u8];
        let val = Facility::try_from(&data[..]).unwrap();
        ...
        assert_eq!(val.status, FacilityStatus::default());
    }

    #[test]
    fn test_state_facility_serialization() {
        let val = Facility {
        ...
        let val2 = Facility::try_from(&data[..]).unwrap();
    ...
    #[test]
    fn test_state_facility_validate_error_invalid_account_type() {
        let val = Facility {
            account_type: AccountType::Device, // Should be Facility
            owner: Pubkey::new_unique(),
            index: 123,
            bump_seed: 1,
            reference_count: 0,
    ...
    #[test]
    fn test_state_facility_validate_error_code_too_long() {
        let val = Facility {
            account_type: AccountType::Facility,
            owner: Pubkey::new_unique(),
            index: 123,
    ...
    #[test]
    fn test_state_facility_validate_error_name_too_long() {
        let val = Facility {
            account_type: AccountType::Facility,
            owner: Pubkey::new_unique(),
            index: 123,
    ...
    #[test]
    fn test_state_facility_validate_error_invalid_country_code() {
        let val = Facility {
            account_type: AccountType::Facility,
            owner: Pubkey::new_unique(),
            index: 123,
    ...
    #[test]
    fn test_state_facility_validate_error_invalid_latitude() {
        let val_low = Facility {
            account_type: AccountType::Facility,
            owner: Pubkey::new_unique(),
            index: 123,
            bump_seed: 1,
    ...
    #[test]
    fn test_state_facility_validate_error_invalid_longitude() {
        let val_low = Facility {
            account_type: AccountType::Facility,
            owner: Pubkey::new_unique(),
            index: 123,
