...
func TestLoadOrMigrateState_MigrationFromOldFile(t *testing.T) {
	dir := t.TempDir()
	oldPath := filepath.Join(dir, oldStateFileName)

	// Create old doublezerod.json
	...
	if !enabled {
		t.Fatal("expected enabled=true after migration")
	}
...
func TestLoadOrMigrateState_MigrationWithRealContent(t *testing.T) {
	dir := t.TempDir()
	oldPath := filepath.Join(dir, oldStateFileName)

	// Old doublezerod.json might have had real provisioning data
	...
	if !enabled {
		t.Fatal("expected enabled=true after migration from old file with content")
	}
