...
func TestE2E_ContributorAuth(t *testing.T) {
	...
	if !t.Run("contributor_owner_interface_update", func(t *testing.T) {
		...
		_, err = dn.Manager.Exec(t.Context(), []string{
			"solana", "transfer", coOwnerPubkey, "10",
		})
		...
		_, err = dn.Manager.Exec(t.Context(), []string{
			"solana", "transfer", randomPubkey, "10",
		})
	...
	t.Run("contributor_owner_update_ops_manager", func(t *testing.T) {
		...
		_, err = dn.Manager.Exec(t.Context(), []string{"bash", "-c", fmt.Sprintf(`
			set -euo pipefail
			DOUBLEZERO_KEYPAIR=/tmp/co-owner.json doublezero contributor update --pubkey test-co04 --ops-manager %s 2>&1
		`, opsMgrPubkey)})
		...
		_, err = dn.Manager.Exec(t.Context(), []string{"bash", "-c", `
			set -euo pipefail
			DOUBLEZERO_KEYPAIR=/tmp/co-owner.json doublezero contributor update --pubkey test-co04 --code new-code 2>&1
		`})
