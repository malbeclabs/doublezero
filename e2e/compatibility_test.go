		...
		t.Run(testName, func(t *testing.T) {
			...
			t.Run("manager_write", func(t *testing.T) {
				...
				writeSteps := []writeStep{
					...
					// Set minimum compatible version (requires special permissions, may fail).
					{name: "global_config_set_version", cmd: cli + " global-config set-version --min-compatible-version 0.0.1", noCascade: true},
