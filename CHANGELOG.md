...
- Serviceability
  ...
  - `CreateUser` and `CreateSubscribeUser` validate an optional RFC-27 `IpOwnershipProof`, so a caller can no longer bind a `client_ip` it cannot originate traffic from. A wildcard access pass — one stored at `0.0.0.0` or flagged `allow_multiple_ip`, which includes the `EdgeSeat` passes the shred-oracle issues — accepts any globally-routable address today, which lets a registrant squat the `(client_ip, user_type)` User PDA of an address someone else operates and point that device's tunnel and route provisioning at a third party. BPF cannot verify Ed25519 cheaply, so the transaction carries a native `Ed25519SigVerify` instruction and the program introspects the Instructions sysvar to confirm it covers the message this creation implies, signed by `globalstate.ip_verifier_authority_pk`. The program scans for that instruction rather than reading a fixed index, so a client may place it anywhere and interleave compute-budget instructions; it rejects any whose precompile offsets name a different instruction, run past the end of its data, or that carries other than exactly one signature — an offset pointing elsewhere means the precompile verified bytes the program never reads. The signed message binds `user_type` alongside the payer, IP, and epoch — the User PDA is `f(client_ip, user_type)`, so a proof obtained for a routine connect cannot be replayed into a different connection type on the same address — and the epoch window is `clock.epoch` or `clock.epoch - 1` so a proof fetched just before a rollover still lands. A proof whose layout version the program does not support is rejected rather than reconstructed against the version it does know. An unconfigured verifier key is a hard reject, never "any signature passes". Enforcement is gated on the new `require-ip-ownership-proof` feature flag (bit 2): while it is clear a missing proof is accepted so pre-RFC-27 clients keep working, but a supplied proof is validated in full either way, because a client attaching a broken proof is broken now rather than at rollout. With the flag set a proof is required for every creation, wildcard and specific-IP passes alike, including the idempotent rerun path — otherwise an account squatted before the flag was set would absorb every later call without its holder ever demonstrating control. The Instructions sysvar rides as an optional account appended last and peeled off the tail by its fixed key, so `CreateUser`'s length-detected tenant slot and `CreateSubscribeUser`'s PDA-matched trailing Permission account are untouched. Costs about 1,300 CU against a 1,400,000 budget. Nothing carries the proof yet; the CLI and SDK work is tracked separately. (#4197)
- Utility crates
