...
func ReplaceTableViaCSV(ctx context.Context, log *slog.Logger, conn Connection, tableName string, count int, writeCSVFn func(*csv.Writer, int) error, primaryKeyColumns []string) error {
	...
	if count == 0 {
		...
		return retryWithBackoff(ctx, log, fmt.Sprintf("replace table %s (empty)", tableName), func() error {
			select {
			case <-ctx.Done():
	...
	return retryWithBackoff(ctx, log, fmt.Sprintf("replace table %s", tableName), func() error {
		select {
		case <-ctx.Done():
			return fmt.Errorf("context cancelled before transaction for %s: %w", tableName, ctx.Err())
		default:
		...
		// COPY same CSV file - idempotent because MERGE operations below are idempotent
		...
		// Do UPDATE/INSERT first (like upsert), then DELETE separately
		// DuckLake only supports a single UPDATE/DELETE action per MERGE statement
		// These MERGE operations are idempotent - running multiple times with the same data
		...
		// Delete rows not in source (separate MERGE operation)
		// This is idempotent - if rows were already deleted, this is a no-op
