...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Listen for HTMX content insertion to resize new operator selects
    document.body.addEventListener('htmx:afterSwap', function(e) {
        // Resize any new operator selects in the swapped content
        const newSelects = e.detail.target.querySelectorAll('.operator-select');
        newSelects.forEach(selectEl => resizeOperatorSelect(selectEl));
    });
});
...
// Initialize ECharts (lazy - actual init happens in renderChart)
function initializeChart() {
    // Chart is initialized lazily in renderChart() to avoid memory leaks
    // Just ensure chart variable is null initially
    chart = null;
}
...
// Operator change handler - hide value input for IS EMPTY / IS NOT EMPTY
function onOperatorChange(selectEl) {
    ...
    // Resize the operator dropdown to fit content
    resizeOperatorSelect(selectEl);
}

// Resize operator select to fit selected option text
function resizeOperatorSelect(selectEl) {
    // Create a hidden measurement span if it doesn't exist
    let measureSpan = document.getElementById('operator-measure-span');
    if (!measureSpan) {
        measureSpan = document.createElement('span');
        measureSpan.id = 'operator-measure-span';
        measureSpan.style.cssText = 'position: absolute; visibility: hidden; white-space: nowrap; font-size: 0.875rem; font-family: inherit; padding: 0.5rem 0.75rem;';
        document.body.appendChild(measureSpan);
    }

    // Measure the selected option text
    const selectedOption = selectEl.options[selectEl.selectedIndex];
    measureSpan.textContent = selectedOption ? selectedOption.text : '';

    // Set width: text width + dropdown arrow padding (2rem) + border (2px)
    const textWidth = measureSpan.offsetWidth;
    const totalWidth = textWidth + 32 + 2; // 32px for arrow area, 2px for borders
    selectEl.style.width = totalWidth + 'px';
}

// Initialize all operator selects on page
function initOperatorSelectWidths() {
    document.querySelectorAll('.operator-select').forEach(selectEl => {
        resizeOperatorSelect(selectEl);
    });
}
...
function collectQueryParams() {
    ...
    // Top N (only applies when group by is set)
    const topNSelect = document.getElementById('top-n');
    const topN = topNSelect ? parseInt(topNSelect.value, 10) : 0;
...
function renderChart(series) {
    ...
    // Ensure chart instance exists (avoid memory leak by reusing)
    if (!chart) {
    ...
    if (!series || series.length === 0) {
        try {
            ...
            chart.setOption({
                title: {
                    text: 'No data returned for the selected query',
                    left: 'center',
                    top: 'center',
            ...
            console.error('Failed to render empty chart state:', e);
        }
    ...
    try {
        ...
        if (statusEl) {
            statusEl.className = 'query-status error';
            statusEl.textContent = 'Error rendering chart. Please try again.';
        }
        return;
    }
...
function renderSankeyChart(series) {
    ...
    try {
        ...
        console.error('Failed to render Sankey chart:', e);
        const statusEl = document.getElementById('query-status');
        if (statusEl) {
            statusEl.className = 'query-status error';
            statusEl.textContent = 'Error rendering Sankey chart. Try a different chart type.';
        }
        return;
    }
...
async function loadQueryFromHistory(index) {
    ...
    // Set top N
    const topNSelect = document.getElementById('top-n');
    if (topNSelect && params.top_n !== undefined) {
        topNSelect.value = params.top_n.toString();
    }
    ...
    if (params.filters && params.filters.length > 0) {
        for (let i = 0; i < params.filters.length; i++) {
            await addFilterFromHistory(params.filters[i], i);
        }
    }

    // Restore group bys
    if (params.group_by && params.group_by.length > 0) {
        for (let i = 0; i < params.group_by.length; i++) {
            await addGroupByFromHistory(params.group_by[i], i);
        }
    }
...
// Add a filter row from history with populated values
async function addFilterFromHistory(filter, index) {
    const container = document.getElementById('filters-container');

    try {
        // Fetch filter row HTML from server
        const response = await fetch(`/api/filter/add?index=${index}`);
        const html = await response.text();
        ...
        const row = container.querySelector(`#filter-row-${index}`);
        if (!row) return;

        // Set column (using the searchable column input)
        const columnInput = row.querySelector('.column-typeahead-input');
        const hiddenColumn = row.querySelector('.selected-column-value');
        if (columnInput && hiddenColumn) {
            const description = getColumnDescription(filter.column);
            columnInput.value = description;
            columnInput.dataset.columnName = filter.column;
            hiddenColumn.value = filter.column;
        }
        ...
        if (operatorSelect && filter.operator) {
            ...
            resizeOperatorSelect(operatorSelect);
        }

        // Hide value input for IS EMPTY / IS NOT EMPTY
        const valueContainer = row.querySelector('.filter-value');
        ...
        const valueInput = row.querySelector('.filter-value .typeahead-input');
        if (valueInput) {
            valueInput.dataset.column = filter.column;

            if (filter.operator === 'IN' || filter.operator === 'NOT IN') {
                // Add as badges for multi-value operators
                const selectedContainer = row.querySelector('.selected-values');
                if (selectedContainer && filter.values) {
                ...
                // Single value
                valueInput.value = filter.values[0];
            }
        }
    } catch (e) {
...
// Add a group by row from history with populated value
async function addGroupByFromHistory(column, index) {
    const container = document.getElementById('groupby-container');

    try {
        // Fetch groupby row HTML from server
        const response = await fetch(`/api/groupby/add?index=${index}`);
        const html = await response.text();
        ...
        // Find the newly added row
        const row = container.querySelector(`#groupby-row-${index}`);
        if (!row) return;
        ...
        console.error('Failed to restore group by:', e);
    }
}
