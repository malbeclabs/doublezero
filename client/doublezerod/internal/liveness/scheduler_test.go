...
func TestClient_Liveness_Scheduler_Run_SendsAndReschedules(t *testing.T) {
	...
	log := newTestLogger(t)
	s := NewScheduler(log, w, func(*Session) {}, 0, false, newMetrics(), false, mustParseClientVersion(t, "1.2.3-dev"))
...
func TestClient_Liveness_Scheduler_Run_CullsStaleDetectAndClearsMarker(t *testing.T) {
	...
	log := newTestLogger(t)
	s := NewScheduler(log, nil, func(*Session) {}, 0, false, newMetrics(), false, mustParseClientVersion(t, "1.2.3-dev"))
	ctx, cancel := context.WithCancel(t.Context())
	defer cancel()
...
func TestClient_Liveness_Scheduler_doTX_RespectsContextCancelOnWriteError(t *testing.T) {
	...
	s := NewScheduler(log, udp, func(*Session) {}, 0, false, newMetrics(), false, mustParseClientVersion(t, "1.2.3-dev"))
...
func TestClient_Liveness_Scheduler_doTX_SetsPassiveBitWhenPassiveModeEnabled(t *testing.T) {
	...
	s := NewScheduler(newTestLogger(t), w, func(*Session) {}, 0, false, newMetrics(), true, mustParseClientVersion(t, "1.2.3-dev"))
...
func TestClient_Liveness_Scheduler_doTX_SetsClientVersionOnPacket(t *testing.T) {
	t.Parallel()
	...
	// This is the version we expect to see encoded in the ControlPacket.
	clientVersion := mustParseClientVersion(t, "7.8.9-beta")

	// passiveMode=false here; what we care about is that ClientVersion is set on the packet.
	s := NewScheduler(
		newTestLogger(t),
		w,
		func(*Session) {},
	...
	require.Equal(t, clientVersion, cp.ClientVersion, "expected ClientVersion on packet to match scheduler's configured version")
	require.Equal(t, "7.8.9-beta", cp.ClientVersion.String(), "sanity check on encoded ClientVersion string")
}
...
func mustParseClientVersion(t *testing.T, s string) ClientVersion {
	v, err := ParseClientVersion(s)
	require.NoError(t, err)
	return v
}
