...
impl ListMulticastGroupPubAllowlistCliCommand {
    pub fn execute<C: CliCommand, W: Write>(self, client: &C, out: &mut W) -> eyre::Result<()> {
        ...
        let mga_displays = list_accesspass
            .into_iter()
            .filter(|(_, accesspass)| accesspass.mgroup_pub_allowlist.contains(&mgroup_pubkey))
...
mod tests {
    ...
    use crate::{
        multicastgroup::allowlist::publisher::list::ListMulticastGroupPubAllowlistCliCommand,
    ...
    #[test]
    fn test_cli_multicast_publisher_allowlist_list() {
        let mut client = create_test_client();

        let user_payer = Pubkey::from_str_const("11111115q4EpJaTXAZWpCg3J2zppWGSZ46KXozzo4");
