...
- Internet latency collector
  - Rotate a RIPE Atlas target probe that replies but drops most pings. The staleness check only asked whether a measurement exported anything in the last hour, so a target answering a small fraction of the time looked healthy indefinitely and its circuits took turns falling out of the freshness window. Ping outcomes now accumulate in a rolling window and a target above the loss threshold is marked unresponsive, which hands it to the existing rotation path.
  - Prefer a directly reachable probe over one behind NAT when choosing a target, since a NAT'd probe answers for the CPE rather than the probe. The preference runs after responsiveness filtering, so a location whose every direct probe is blacklisted still falls back to a NAT'd one rather than going dark. Source selection is unchanged; NAT does not impede outbound pings.
  - A probe that fails as a measurement target no longer bars itself from sourcing measurements. Target failures go to their own exclusion list and are honored by target selection only; a probe that stopped running measurements altogether still bars both. Failing to answer pings says nothing about sending them, so the old behavior dropped a location's circuits to every other target for no reason, and a location whose sole probe failed as a target lost its circuits entirely.
- Serviceability
