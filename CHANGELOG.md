...
- SDK
  ...
  - Migrate every serviceability `commands/*` `execute()` to delegate to those builders and a new `DoubleZeroClient::send_transaction` (compute-budget prelude + sign + send), replacing the four `execute_*(instruction, accounts)` methods and the client-side account assembly. **Behavior change:** the Rust SDK no longer attaches the payer's Permission PDA to serviceability transactions — `authorize()` uses the legacy GlobalState allowlist path until the builder-side Permission append is re-enabled per instruction (must be sequenced with the permission-model rollout). Command `execute()` signatures/returns are unchanged, so CLI/sentinel/daemon consumers are unaffected.
