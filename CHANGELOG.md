...
- Serviceability
  ...
  - An EdgeSeat access pass can now hold a user on more than one feed. Joining a group from a second feed on the pass takes that feed's seat, and leaving the feed's last group gives the seat back. One seat per feed, however many of the feed's groups the user joins, charged only by the call that first joins a group so a group two feeds both carry cannot be billed twice. Whether a role change counts as an add or a removal is decided against the user's current membership, so dropping one role while keeping the other is not re-gated and a call that changes nothing neither charges nor frees a seat. New errors: `UserDeviceMismatch` (101) when the device passed to a role change is not the user's, `UserFeedLimitExceeded` (104) at the per-user feed cap, and `DeviceAccountRequired` (105) in place of a misleading `MetroMismatch` when the device account is missing. (#4099)
- Client daemon
