...
use clap::Args;
use doublezero_sdk::{
    commands::{link::list::ListLinkCommand, topology::list::ListTopologyCommand},
    get_topology_pda,
};
use serde::Serialize;
use std::io::Write;
...
impl ListTopologyCliCommand {
    pub fn execute<C: CliCommand, W: Write>(self, client: &C, out: &mut W) -> eyre::Result<()> {
        ...
        let displays: Vec<TopologyDisplay> = entries
            .iter()
            .map(|(pda, t)| {
                // Count how many links reference this topology PDA. The topology_pda
                // is derived from name, but we already have the PDA as the map key.
