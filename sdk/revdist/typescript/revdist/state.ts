...
export function deserializeComputedSolanaValidatorDebts(
  ...
  for (let i = 0; i < count; i++) {
    const nodeId = new PublicKey(r.readPubkeyRaw());
...
export function deserializeShapleyOutputStorage(
  ...
  for (let i = 0; i < count; i++) {
    const contributorKey = new PublicKey(r.readPubkeyRaw());
    const unitShare = r.readU32();
    const remainingBytes = r.readBytes(4);
    rewards.push({ contributorKey, unitShare, remainingBytes });
  }
  const totalUnitShares = r.readU32();
