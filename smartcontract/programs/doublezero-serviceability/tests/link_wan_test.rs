...
/// Operational drain controls: an Activated WAN link can be cycled through
/// HardDrained / SoftDrained and back to Activated via UpdateLink. Verifies
/// the drain surface independent of create/delete (atomic create handled by
/// setup_link_env; this test only exercises status transitions).
#[tokio::test]
async fn test_wan_link_drain_transitions() {
    let (
        mut banks_client,
        program_id,
        payer,
    ...
    // Sanity: link starts Activated from atomic create.
    let link = get_account_data(&mut banks_client, tunnel_pubkey)
        .await
        .expect("Link not found")
        .get_tunnel()
