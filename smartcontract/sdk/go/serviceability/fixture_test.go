...
func TestFixtureFeed(t *testing.T) {
	...
	require.Len(t, feed.Metros, 2)
	assert.Equal(t, byte(0xE1), feed.Metros[0].Exchange[0])
	require.Len(t, feed.Metros[0].Groups, 2)
	assert.Equal(t, byte(0xE2), feed.Metros[0].Groups[0][0])
	assert.Equal(t, byte(0xE3), feed.Metros[0].Groups[1][0])
	assert.Equal(t, byte(0xE4), feed.Metros[1].Exchange[0])
	require.Len(t, feed.Metros[1].Groups, 1)
	assert.Equal(t, byte(0xE5), feed.Metros[1].Groups[0][0])
}
