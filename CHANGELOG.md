...
- CLI
  - `doublezero access-pass migrate-to-oracle` re-owns the shred oracle's validator-seeded access passes to the oracle. For each pass where `owner == oracle` but `user_payer != oracle`, it packs the whole re-own into one atomic transaction per pass — unsubscribe the Multicast user from its groups, delete it, close the old validator-seeded pass, recreate the pass oracle-seeded, restore its subscriber allowlist, and recreate + re-subscribe the user oracle-owned — so the client's `User` account and tunnel never move. Dry run by default (assembles and simulates each transaction and reports its serialized size); `--execute` sends. Must be signed by the oracle key. A pass still shared by another (non-migrated) user is migrated but left open rather than closed. (#4073)
