...
func TestGlobalMonitor_Solana_ValidatorsView_New_NilArgs(t *testing.T) {
	...
	_, err := NewValidatorsView(nil, rpc, geo)
	require.Error(t, err)

	_, err = NewValidatorsView(logger, nil, geo)
	require.Error(t, err)

	_, err = NewValidatorsView(logger, rpc, nil)
	require.Error(t, err)
}
...
func TestGlobalMonitor_Solana_ValidatorsView_New_Success(t *testing.T) {
	...
	v, err := NewValidatorsView(logger, rpc, geo)
...
func TestGlobalMonitor_Solana_ValidatorsView_GetByNodePubkey_Success(t *testing.T) {
	...
	geo := &MockGeoIPResolver{
		ResolveFunc: func(ip net.IP) *geoip.GeoIP {
			resolvedIP = ip
			return nil
		},
	}

	view, err := NewValidatorsView(logger, rpc, geo)
	require.NoError(t, err)
	...
	require.NotNil(t, v.Node.IP)
	require.Equal(t, "1.2.3.4", v.Node.IP.String())
	require.Equal(t, uint16(9000), v.Node.TPUQUICPort)

	// LeaderRatio = 5 / 10 = 0.5
	require.InDelta(t, 0.5, v.LeaderRatio, 1e-9)

	// GeoIP called
	require.NotNil(t, resolvedIP)
	require.Equal(t, v.Node.IP.String(), resolvedIP.String())
...
func TestGlobalMonitor_Solana_ValidatorsView_LeaderScheduleError(t *testing.T) {
	...
	view, _ := NewValidatorsView(logger, rpc, &MockGeoIPResolver{})
...
func TestGlobalMonitor_Solana_ValidatorsView_gossipNodeFromRPC_ParsesIPv4AndPort(t *testing.T) {
	...
	logger := newTestLogger()
	view, _ := NewValidatorsView(logger, &MockSolanaRPC{}, &MockGeoIPResolver{})
	...
	require.NotNil(t, g.IP)
	require.Equal(t, "5.6.7.8", g.IP.String())
...
func (m *MockGeoIPResolver) Resolve(ip net.IP) *geoip.GeoIP {
