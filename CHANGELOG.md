...
- Collector
  - A RIPE Atlas measurement keeps the target probe it is using unless that probe is marked unresponsive as a target, has not exported for over an hour, or is no longer among the location's candidates. Target selection ranks only on distance and the marks, so when a nearer probe's 24h mark expired the pick flipped back to it and the measurement was rebuilt onto a probe whose quality then had to be re-learned by running it for hours. Columbus (`cmh`) went around that loop daily and lost roughly four hours of telemetry to it. (malbeclabs/doublezero#4362)
- CLI
