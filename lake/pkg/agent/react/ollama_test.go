...
func TestNewOllamaAgent(t *testing.T) {
	...
	// System prompt is enhanced with ollamaToolUseDirective
	assert.Contains(t, ollamaAgent.system, "You are a helpful assistant.")
	assert.Contains(t, ollamaAgent.system, "CRITICAL INSTRUCTION")
}
...
func TestOllamaAgent_Call_WithToolCall(t *testing.T) {
	...
	content := response.Content()
	require.Len(t, content, 2) // tool call + text

	// Check tool call (comes first in Content())
	id, name, input, ok := content[0].AsToolUse()
	assert.True(t, ok)
	...
	// Check text (comes second)
	text, ok := content[1].AsText()
	assert.True(t, ok)
...
func TestOllamaJSONArgs_UnmarshalJSON(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, json.Unmarshal(args.Raw(), &actual))
