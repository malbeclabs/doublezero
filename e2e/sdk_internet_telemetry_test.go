...
func TestE2E_SDK_Telemetry_InternetLatencySamples(t *testing.T) {
	...
	t.Run("try to get internet latency samples before initialized", func(t *testing.T) {
		...
		internetLatencySamples, err := telemetryClient.GetInternetLatencySamples(ctx, dataProvider1Name, laxLocationPK, laxLocationPK, epoch)
	...
	t.Run("get internet latency samples after initialized", func(t *testing.T) {
		...
		account, err := telemetryClient.GetInternetLatencySamples(ctx, dataProvider1Name, laxLocationPK, amsLocationPK, epoch)
		require.NoError(t, err)
	...
	t.Run("get internet latency samples after writing second", func(t *testing.T) {
		...
		start := time.Now()
		log.Info("==> Getting internet latency samples from PDA")
		internetLatencySamples, err := telemetryClient.GetInternetLatencySamples(ctx, dataProvider1Name, laxLocationPK, amsLocationPK, epoch)
		require.NoError(t, err)
