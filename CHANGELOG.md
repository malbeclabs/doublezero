...
- Tools
  ...
  - Implement the `tools/stress/device-observer/` Prometheus scraper for the doublezero-agent metrics endpoint. Every tick fetches `--agent-metrics-url`, parses the exposition response, and appends one NDJSON row per metric sample to `observer.agent_metrics.json` (counters/gauges plus `_sum`/`_count`/`_bucket` suffixes for summaries and histograms; labels stored as a compact JSON-encoded string for line-oriented consumers). Counter family totals are also exposed via a thread-safe `Scraper.Snapshot()` for the abort decider (PR #3796) to detect mid-sample counter increments. Per-tick HTTP, parse, or write failures log at WARN and the loop continues — repeated-failure policy stays with the abort decider. Replaces the no-op collector stub from the scaffolding PR.
- E2E/QA
