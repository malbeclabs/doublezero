...
impl GetMulticastGroupCliCommand {
    pub fn execute<C: CliCommand, W: Write>(self, client: &C, out: &mut W) -> eyre::Result<()> {
        ...
        for (pubkey, user) in users.into_iter().filter(|(_, user)| {
            user.publishers.contains(&mgroup.owner) || user.subscribers.contains(&mgroup.owner)
        }) {
            ...
            let mode_text = if user.publishers.contains(&pubkey) {
                if !user.subscribers.contains(&pubkey) {
                    "Tx"
                } else {
                    "Tx/Rx"
                }
            } else if user.subscribers.contains(&pubkey) {
                "Rx"
            } else {
                "XX"
            };
