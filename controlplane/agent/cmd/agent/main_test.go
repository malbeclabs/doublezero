	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if shouldApply != tt.shouldApply {
				t.Errorf("%s: expected %v, got %v", tt.description, tt.shouldApply, shouldApply)
			}
		})
	}
}
...
func TestConfigPollingIntegration(t *testing.T) {
	// This test simulates multiple polling cycles to verify the caching behavior

	type pollingCycle struct {
		configText    string
		expectApply   bool
	...
	cycles := []pollingCycle{
		{
			configText:    "config version 1",
			expectApply:   true,
			timeSinceLast: 0,
			description:   "First poll - should always apply",
		},
		{
			configText:    "config version 1",
			expectApply:   false,
			timeSinceLast: 5 * time.Second,
			description:   "Same config after 5s - should not apply",
		},
		{
			configText:    "config version 2",
			expectApply:   true,
			timeSinceLast: 5 * time.Second,
			description:   "Changed config - should apply",
		},
		{
			configText:    "config version 2",
			expectApply:   false,
			timeSinceLast: 30 * time.Second,
			description:   "Same config after 30s total - should not apply",
		},
		{
			configText:    "config version 2",
			expectApply:   true,
			timeSinceLast: 31 * time.Second, // Total 61s since last apply
			description:   "Same config after 61s total - should apply due to timeout",
		},
	}
	...
	for i, cycle := range cycles {
		t.Run(cycle.description, func(t *testing.T) {
			if i > 0 {
				currentTime = currentTime.Add(cycle.timeSinceLast)
			}

			newHash := computeChecksum(cycle.configText)
			...
			if cachedHash == "" {
				...
				shouldApply = true
			} else if currentTime.Sub(cacheTime) >= cacheTimeout {
				shouldApply = true
			}

			if shouldApply != cycle.expectApply {
				t.Errorf("%s: expected %v, got %v", cycle.description, cycle.expectApply, shouldApply)
			}

			// Update cache if we applied
			if shouldApply {
				cachedHash = newHash
