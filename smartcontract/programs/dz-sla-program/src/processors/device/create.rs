...
use crate::{error::DoubleZeroError, helper::*, state::{accounttype::AccountType, device::*}};
use crate::pda::*;
use crate::types::*;
...
impl fmt::Debug for DeviceCreateArgs {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "code: {}, location_pk: {}, exchange_pk: {}, device_type: {:?}, public_ip: {}, dz_ef_pools: {}",
            self.code, self.location_pk, self.exchange_pk, self.device_type, ipv4_to_string(&self.public_ip), networkv4_list_to_string(&self.dz_ef_pools)
        )
    }
}
...
pub fn process_create_device(
    ...
    // Check account Types
    if location_account.data_is_empty() || location_account.data.borrow()[0] != AccountType::Location as u8 {
        return Err(DoubleZeroError::InvalidLocationPubkey.into());
    }
    if location_account.owner != program_id {
        return Err(ProgramError::IncorrectProgramId);
    }     
    if exchange_account.data_is_empty() || exchange_account.data.borrow()[0] != AccountType::Exchange as u8 {
    ...
    let device: Device = Device {
        ...
        dz_ef_pools: value.dz_ef_pools.clone(),
