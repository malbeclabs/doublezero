...
- Serviceability
  - An EdgeSeat access pass now admits multicast users only. A unicast connect is rejected with `EdgeSeatIsMulticastOnly` (variant 103) instead of being admitted against `max_unicast_users`, which is no longer enforced and is retained for layout compatibility only. EdgeSeat passes exist to sell feed seats, and a unicast user has no feed to bill against. The feed-subscription flow already set `max_unicast_users: 0`, so its passes rejected unicast connects already; a pass created by hand (the CLI flag defaults to 1) could admit one before this change. The same rule applies to role adds: an EdgeSeat pass grants a multicast role only to a `UserType::Multicast` user, since a seat is per feed and no other type takes one. Other pass types are unaffected and still allow a multicast role on any user type, gated by the multicast-group allowlist. Removal stays permitted everywhere, so any user already in one of these states can be cleaned up. (#4099)
...
- Serviceability
  ...
  - An EdgeSeat access pass can now hold a user on more than one feed. Joining a group from a second feed on the pass takes that feed's seat, and leaving the feed's last group gives the seat back. One seat per feed, however many of the feed's groups the user joins, charged only by the call that first joins a group so a group two feeds both carry cannot be billed twice. Whether a role change counts as an add or a removal is decided against the user's current membership, so dropping one role while keeping the other is neither re-gated nor allowed to free the seat, and a call that changes nothing neither charges nor frees one. A feed is a receive-only SKU, so an EdgeSeat pass grants subscriber roles only and a publisher role is rejected with `EdgeSeatIsSubscribeOnly` (106); dropping one already held stays allowed. New errors: `UserDeviceMismatch` (101) when the device passed to a role change is not the user's, `UserFeedLimitExceeded` (104) at the per-user feed cap, and `DeviceAccountRequired` (105) in place of a misleading `MetroMismatch` when the device account is missing. (#4099)
- CI
...
- SDK
  ...
  - `UpdateMulticastGroupRolesCommand` works on EdgeSeat access passes. It no longer applies the multicast-group allowlist checks those passes bypass onchain, which had rejected every EdgeSeat role change, and it looks up the device and feed the change needs. `update_multicast_group_roles` takes them as a new optional argument. (#4099)
