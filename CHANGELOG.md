...
- Collector
  - The Wheresitup collector drops tracked jobs that are past their expiry instead of polling them, and asks for a one-hour expiry rather than ten minutes. WheresItUp discards a job's results at `expire_after` and thereafter returns the job with an empty `complete` and a populated `in_progress`, which reads exactly like a job still running — so an expired job was polled on every cycle and held for two hours, yielding nothing. A vendor-side fault left thousands of jobs stuck; polling them stretched the export pass and the collection cycle past ten minutes, so every newly created job was first polled after its own expiry, read as in progress, and was itself retained. That loop sustained itself after the vendor recovered and left mainnet-beta's wheresitup provider reporting no samples for about 22 hours. Skipping jobs older than the expiry plus a minute bounds the poll set to roughly one cycle's worth of jobs regardless of vendor behaviour, and the one-hour expiry — which job creation already claimed in its logs while the payload sent ten minutes — keeps a job's results in place until the next cycle polls them.
