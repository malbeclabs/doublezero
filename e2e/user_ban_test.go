...
func TestE2E_UserBan(t *testing.T) {
	...
	_, err = dn.Manager.Exec(t.Context(), []string{"bash", "-c", "doublezero access-pass set prepaid --client-ip " + client1.CYOANetworkIP + " --user-payer " + client1.Pubkey})
	require.NoError(t, err)
	// Set access pass for the client.
	_, err = dn.Manager.Exec(t.Context(), []string{"bash", "-c", "doublezero access-pass set prepaid --client-ip " + client2.CYOANetworkIP + " --user-payer " + client2.Pubkey})
	require.NoError(t, err)

	// Run IBRL workflow test.
	if !t.Run("user-ban-ibrl", func(t *testing.T) {
		runUserBanIBRLWorkflowTest(t, log, client1, client2, dn)
	}) {
		t.Fail()
	}
}
