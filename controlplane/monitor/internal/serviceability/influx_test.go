...
func TestToLineProtocol(t *testing.T) {
	...
	type testDevice struct {
		...
		Status     string     `influx:"tag,status"`
	...
	testCases := []struct {
		...
		},
		{
			...
			expected: `links,code=link-01,contributor_pubkey=` + pubKey1B58 +
				...
				`,status=activated,tunnel_id=42,tunnel_net=172.16.0.0/24 bandwidth=1000,delay_ns=1e+06,delay_override_ns=2e+06,jitter_ns=500000,link_health="unknown",mtu=1500`,
			expectErr: false,
		},
		{
			...
			expected: `links,code=link-empty,contributor_pubkey=` + pubKey1B58 +
				...
				`,status=pending,tunnel_id=7 bandwidth=0,delay_ns=0,delay_override_ns=0,jitter_ns=0,link_health="unknown",mtu=0`,
			expectErr: false,
		},
		{
			name:        "empty struct",
			...
			input:       testDevice{},
			ts:          ts,
			expected:    `devices,device_type=0,owner=11111111111111111111111111111111,public_ip=0.0.0.0 dz_prefixes="",max_users=0,users_count=0`,
			expectErr:   false,
		},
		{
			...
			expected:  `devices,code=dev-01,device_type=2,env=mainnet,owner=` + pubKey2B58 + `,public_ip=0.0.0.0 dz_prefixes="",max_users=0,users_count=0`,
			expectErr: false,
		},
		{
			name:        "no tags",
		...
		{
			name:        "empty tag value is omitted",
			measurement: "contributors",
			input: testContributor{
				Owner:   pubKey1,
				Status:  "activated",
		...
		{
			name:        "tag value with spaces is escaped",
			measurement: "exchanges",
			input: testExchange{
				Owner:   pubKey1,
				Lat:     10.0,
