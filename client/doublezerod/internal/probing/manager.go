...
var (
	// TODO(snormore): Do we actually see 8.8.8.8 in production or is it just the docker e2e env?
	unmanagedRouteDstIP = net.ParseIP("8.8.8.8")
)
...
func (m *ProbingManager) handleRouteAdd(route *routing.Route) error {
	...
	// If the route is in the unmanaged routes list, we don't manage it.
	if unmanagedRouteDstIP != nil && route.Dst.IP.Equal(unmanagedRouteDstIP) {
		return m.cfg.Netlink.RouteAdd(route)
	}
