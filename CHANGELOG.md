...
- CI
  ...
  - Thread all testnet release orchestrator Slack posts under a single per-run parent message in `#int-tech`, posted via the Slack Web API (`chat.postMessage` with a bot token) instead of incoming webhooks, which cannot start threads. Adds a threaded PR-links post after `open-prs` covering the merge-both-PRs / approve-gate-1 human steps. Slack failures degrade to workflow warnings and flat posts, never failing the release.
