...
- CLI
  - `doublezero feed migrate --chain solana --feed "Shreds NY"` writes the chain on feeds that still have none. `--feed` takes a pubkey or a name, and a name that matches more than one feed fails and asks for the pubkey. A feed that already has Solana or Hyperliquid is printed and skipped. The command packs up to 8 feeds into one ledger transaction and starts another when the packet is full. `--chain unspecified` is refused.
  - `doublezero feed create --chain` and `doublezero feed update --chain` take `solana`, `hyperliquid`, or `unspecified`. Create without the flag writes `Unspecified`. Update without the flag leaves the stored value. `doublezero feed list` prints the chain after the name. (#4353)
- SDK
  - `MigrateFeedCommand` sends one `MigrateFeed` instruction per feed and refuses `Unspecified`. `MAX_FEEDS_PER_TRANSACTION` is 8 so a full batch plus the compute-budget prelude stays under 1232 bytes.
  - `CreateFeedCommand` and `UpdateFeedCommand` carry the chain. Go, Python, and TypeScript decoders read the trailing byte and decode a short account as `Unspecified`. The feed fixture ends with `Hyperliquid` (2). (#4353)
- Serviceability
  - `MigrateFeed` (variant 125) writes the chain on a feed that still has `Unspecified`. A second migrate is `FeedAlreadyMigrated` (131). `Unspecified` as the new value is `InvalidFeedChain` (132). A `FEED_AUTHORITY` or `FOUNDATION` key signs it. Every other field is left as stored.
  - `Feed` carries `feed_chain`: `Unspecified` 0, `Solana` 1, `Hyperliquid` 2. Monthly rewards copy this onto each `FeedDistribution`. A feed written before the field decodes as `Unspecified`, so a later migrate can tell it from a feed that already chose a chain. `CreateFeed` writes the caller's value and accepts `Unspecified`. `UpdateFeed` changes it only when the argument is set. (#4353)
