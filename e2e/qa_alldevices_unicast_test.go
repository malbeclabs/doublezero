...
func TestQA_AllDevices_UnicastConnectivity(t *testing.T) {
	...
	clients := make([]*qa.Client, 0, len(test.Clients()))
	...
	deviceCodes := Map(devices, func(d *qa.Device) string { return d.Code })
	...
	for start := 0; start < len(devices); start += batchSize {
		...
		// Connect all active clients to their assigned devices sequentially
		...
		// Wait for all active clients to be up.
		for _, c := range activeClients {
			err := c.WaitForStatusUp(ctx)
			require.NoError(t, err, "failed to wait for status for client %s", c.Host)
		}

		// Wait for routes between all active clients.
		for _, c := range activeClients {
			expectedRoutes := make([]net.IP, 0, len(activeClients)-1)
			for _, other := range activeClients {
				if other.Host == c.Host {
					continue
				}
				expectedRoutes = append(expectedRoutes, other.PublicIP())
			}
			err := c.WaitForRoutes(ctx, expectedRoutes)
		...
		//   - Tests connectivity from that client to all other active clients
		...
		// After all per-device subtests for this batch, disconnect all active clients.
		func() {
			var wg sync.WaitGroup
			for _, c := range activeClients {
				wg.Add(1)
				go func(c *qa.Client) {
					defer wg.Done()
					e := c.DisconnectUser(context.Background(), true, true)
					assert.NoError(t, e, "failed to disconnect user %s", c.Host)
				}(c)
			}
			wg.Wait()
		}()
