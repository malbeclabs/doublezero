...
func (w *Worker) updatePendingDeviceHealth(ctx context.Context, devices []serviceability.Device, globalStatePubkey solana.PublicKey) {
	...
	for _, device := range devices {
		...
		// Update health for devices with unknown or pending health
		if device.DeviceHealth != serviceability.DeviceHealthPending &&
			device.DeviceHealth != serviceability.DeviceHealthUnknown {
			continue
		}
...
func (w *Worker) updatePendingLinkHealth(ctx context.Context, links []serviceability.Link, globalStatePubkey solana.PublicKey) {
	...
	for _, link := range links {
		...
		// Update health for links with unknown or pending health
		if link.LinkHealth != serviceability.LinkHealthPending &&
			link.LinkHealth != serviceability.LinkHealthUnknown {
			continue
		}
