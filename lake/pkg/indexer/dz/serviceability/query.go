...
func QueryCurrentDevices(ctx context.Context, log *slog.Logger, db clickhouse.DB) ([]Device, error) {
...
func QueryCurrentLinks(ctx context.Context, log *slog.Logger, db clickhouse.DB) ([]Link, error) {
	conn, err := db.Conn(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get connection: %w", err)
	}
...
func QueryCurrentMetros(ctx context.Context, log *slog.Logger, db clickhouse.DB) ([]Metro, error) {
	conn, err := db.Conn(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get connection: %w", err)
	}
...
func QueryCurrentUsers(ctx context.Context, log *slog.Logger, db clickhouse.DB) ([]User, error) {
	conn, err := db.Conn(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get connection: %w", err)
	}
