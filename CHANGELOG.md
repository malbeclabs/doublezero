...
- Client daemon
  - Route liveness reconverges promptly after a transient loss instead of stalling for up to ~3x `-route-liveness-backoff-max`. A state-changing RX now advertises the new state immediately rather than waiting out the transmit timer that was armed while the session was Down (exponentially backed off up to `backoffMax`), so the Down→Init→Up handshake completes in about one interval plus two round trips as RFC-7 specifies. Two defects contributed: the RX path never enqueued a TX on a state change, and the scheduler's sleep had no wakeup path, so an event pushed earlier than its current deadline could not shorten the sleep. Also logs the effective `minTxFloor`/`maxTxCeil`/`backoffMax` at startup, since a wrong value was otherwise only visible as slow reconvergence. (#3935)
- Device controller
