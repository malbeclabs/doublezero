...
- test(contributor-rewards): mark the three keypair tests `#[serial]`. They set and remove `REWARDER_KEYPAIR_PATH`, which is process-wide state, so one removing it while another sat between `set_var` and the read made the read fail. It never fired under `cargo nextest`, which gives each test its own process, and became reachable when this crate joined a workspace whose CI runs `cargo test`
