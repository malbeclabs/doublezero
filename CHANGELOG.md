...
- Serviceability
  - Fix `doublezero link topology clear`, which could never clear a topology that a link actually referenced: the SDK builder passed the topology PDA readonly while the processor must write it to decrement `reference_count`, so every invocation in which a link actually held the reference aborted with `Topology Account is not writable` after the link writes had already been staged. The account list now comes from a single `clear_topology_account_metas` function shared by the SDK builder and the program integration tests, so a client/processor writability divergence fails a test instead of a live transaction. The processor also validates the topology account up front rather than part-way through the link loop; a readonly non-empty topology account is now rejected deterministically instead of only when at least one link held a reference, so an operator on a pre-fix CLI binary sees a hard failure rather than a misleading success in the no-op case. A clear against an already-closed topology now logs that it was a stale-reference cleanup instead of reporting the same line as a live one. (#4091)
