...
- E2E
  - Fix the multicast settlement QA test's seat-withdraw step. The previous ack-wait polled the client seat's pending flag, which cannot distinguish "the oracle has not acked yet" from "no allocation request was created" (a re-fund of an already-active seat creates none), and the single-shot withdraw failed on transient in-flight rejections from lagging RPC reads of the request PDA. Replace the ack-wait with a withdraw that retries while the in-flight rejection persists. (#4065)
