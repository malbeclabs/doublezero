...
- Smartcontract
  - Replace the overloaded `DoubleZeroError::NotAllowed` (custom code 8) rejection in `update_user_multicastgroup_roles` with two specific errors so any client (CLI, Go/Python/TS SDKs) can tell which allowlist a requested multicast group is missing from: `MulticastPubGroupNotInAccessPassAllowlist` (custom code 88) and `MulticastSubGroupNotInAccessPassAllowlist` (custom code 89). The processor `msg!` log now names the offending group pubkey and the relevant allowlist instead of dumping the full `AccessPass`.
...
- Client
  - `doublezero connect ... multicast ...` now pre-flights the requested publisher and subscriber groups against the user's `AccessPass` allowlists and bails before submitting the transaction, with an error message that names every offending group code and the allowlist it is missing from (instead of the previous generic "You are not allowed to execute this action").
