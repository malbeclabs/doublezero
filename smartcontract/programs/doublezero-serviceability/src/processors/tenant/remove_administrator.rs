...
pub struct TenantRemoveAdministratorArgs {
    pub administrator: Pubkey,
}

impl fmt::Debug for TenantRemoveAdministratorArgs {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "administrator: {}", self.administrator)
    }
}

pub fn process_remove_administrator_tenant(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    value: &TenantRemoveAdministratorArgs,
) -> ProgramResult {
    ...
    #[cfg(test)]
    msg!("process_remove_administrator_tenant({:?})", value);
    ...
    if let Some(pos) = tenant
        .administrators
        .iter()
        .position(|x| *x == value.administrator)
    {
        tenant.administrators.remove(pos);
    } else {
        return Err(DoubleZeroError::AdministratorNotFound.into());
    }
