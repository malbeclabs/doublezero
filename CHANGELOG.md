...
- Telemetry
  - A ledger RPC outage no longer stops TWAMP probing on the device telemetry agent. `Pinger.Tick` fetched the current epoch before anything else and returned early on failure, so one unreachable endpoint cost up to 19 hours of latency samples per device across 23 mainnet-beta devices during the 2026-07-29 outage (#4130) — for probes that are pure UDP and would have succeeded throughout, since the epoch only builds the sample buffer's partition key. The pinger now caches the last known epoch and refreshes it on its own loop, so the probe path never touches RPC. Moving the fetch off that path matters as much as the fallback itself: a failing fetch burns ~130s across its retries and `time.Ticker` buffers only one tick, so an inline fetch swallowed roughly a dozen probe opportunities per failure, and caching alone would have resumed probing at about a thirteenth of the sample rate. The refresh cadence defaults to `-probe-interval`, leaving the epoch RPC rate unchanged. Probing is refused only when no epoch has ever been fetched, or when the cached one is older than the new `-max-epoch-staleness` (default 24h). That bound is load-bearing because stale-epoch samples are not discarded as previously assumed: `Submitter.Tick` removes a past-epoch partition only when it is empty, and the program treats the epoch as PDA seed material without validating it against the clock, so they land in the previous epoch's account where the lake's derived `event_ts` places them past the real epoch boundary. Both refusal cases log once rather than per tick, and repeated fetch failures collapse into fresh/stale transitions — the per-attempt retry warning drops to Debug, since at a 10s cadence it emitted thousands of lines across the outage. A new `pinger_epoch_unavailable` error type and a `doublezero_device_telemetry_agent_epoch_cache_stale_age_seconds` gauge make the fallback visible. Recovery is still capped by the 4096-sample partition buffer, so a 19-hour outage recovers roughly 4096 of ~6840 samples rather than none; the remainder is #4126. ([#4125](https://github.com/malbeclabs/doublezero/issues/4125))
