...
func runIBRLTest(t *testing.T, userType api.UserType, provisioningRequest map[string]any) {
	...
	t.Run("IBRL", func(t *testing.T) {
		...
		go func() {
			err := runtime.Run(ctx, sockFile, "", false, false, false, false, newTestNetworkConfig(t), 30, 30, newTestLivenessManagerConfig(), "", 10, 60, t.TempDir())
			errChan <- err
		}()
...
func TestRuntime_Run_PropagatesLivenessStartupError(t *testing.T) {
	...
	sockFile := filepath.Join(rootPath, "doublezerod.sock")
	go func() {
		err := runtime.Run(ctx, sockFile, "", false, false, false, false, newTestNetworkConfig(t), 30, 30, &bad, "", 10, 60, t.TempDir())
		errChan <- err
	}()
...
func TestRuntime_Run_PropagatesLivenessError_FromUDPClosure(t *testing.T) {
	...
	// Start the runtime.
	go func() {
		errCh <- runtime.Run(ctx, sockFile, "", false, false, false, false, newTestNetworkConfig(t), 30, 30, cfg, "", 10, 60, t.TempDir())
	}()
